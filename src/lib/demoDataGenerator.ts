...
function futureDate(daysFromNow: number, hour = 15): string {
  const d = new Date();
...
export async function generateDemoData(
  ...
  const events = [
    {
      ...
      sport, date: futureDate(7, 15),
      lat: DEFAULT_LAT, lng: DEFAULT_LNG,
      city: 'Brest', venue: 'Stade exemple',
      event_type: 'championship', home_or_away: 'home',
      adversaire: 'Club Adversaire FC', team_name: 'Équipe A',
      club_id: clubId, user_id: userId, source: 'user', is_demo: true,
    },
    {
      title: `${name} — Coupe régionale`,
      sport, date: futureDate(14, 14),
      lat: DEFAULT_LAT + 0.05, lng: DEFAULT_LNG + 0.05,
      city: 'Brest', venue: 'Stade municipal',
      event_type: 'cup', home_or_away: 'away',
      adversaire: 'AS Exemple', team_name: 'Équipe A',
      club_id: clubId, user_id: userId, source: 'user', is_demo: true,
    },
    {
      title: `${name} — Match amical`,
      sport, date: futureDate(21, 10),
      lat: DEFAULT_LAT, lng: DEFAULT_LNG,
      city: 'Brest', venue: 'Terrain annexe',
      event_type: 'friendly', home_or_away: 'home',
      adversaire: 'Club Amical', team_name: 'Équipe B',
      club_id: clubId, user_id: userId, source: 'user', is_demo: true,
    },
    {
      title: `Entraînement ${name}`,
      sport, date: futureDate(3, 18),
      lat: DEFAULT_LAT, lng: DEFAULT_LNG,
      city: 'Brest', venue: "Terrain d'entraînement",
      event_type: 'friendly', team_name: 'Seniors',
      club_id: clubId, user_id: userId, source: 'user', is_demo: true,
    },
    {
      title: `Entraînement jeunes ${name}`,
      sport, date: futureDate(5, 17),
      lat: DEFAULT_LAT + 0.01, lng: DEFAULT_LNG,
      city: 'Brest', venue: 'Complexe sportif',
      event_type: 'friendly', team_name: 'U13',
      club_id: clubId, user_id: userId, source: 'user', is_demo: true,
    },
  ];
  ...
  const { error: annErr } = await supabase
    ...
    .insert({
      club_id: clubId, author_id: userId,
      type: 'info',
  ...
  if (createdEvents?.[0]?.id) {
    const { error: rideErr } = await supabase
      ...
      .insert({
        ...
        driver_id: userId, driver_name: 'Exemple Conducteur',
        departure_location: 'Place de la Liberté, Brest',
        departure_lat: DEFAULT_LAT, departure_lng: DEFAULT_LNG,
...
export async function deleteDemoData(clubId: string): Promise<void> {
  ...
  if (eventIds?.length) {
    await supabase.from('rides').delete()
      .in('event_id', eventIds.map((e: { id: string }) => e.id))
      .eq('is_demo', true);
  }
...
export async function hasDemoData(clubId: string): Promise<boolean> {
