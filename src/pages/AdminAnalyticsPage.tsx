...
const CHART_COLORS: Record<string, { solid: string; dim: string }> = {
...
interface TrendBadgeProps { pct: number | null | undefined; }
...
interface EmptyChartProps { message?: string; }
function EmptyChart({ message = 'Données en cours de collecte…' }: EmptyChartProps) {
  return (
...
interface AdminAnalyticsPageProps { onBack: () => void; }
...
export default function AdminAnalyticsPage({ onBack }: AdminAnalyticsPageProps) {
  ...
  const labels: string[] = data?.dayList?.map(shortDay) ?? [];

  function lineDs(curve: any, color: { solid: string; dim: string }) {
    return {
      labels,
      datasets: [{
        data: curve?.map((r: any) => r.count) ?? [],
  ...
  function barDs(curve: any, color: { solid: string; dim: string }) {
    return {
      labels,
      datasets: [{
        data: curve?.map((r: any) => r.count) ?? [],
  ...
  const featureKeys = Object.keys(data?.featureCount ?? {}).sort((a: string, b: string) => (data.featureCount[b] - data.featureCount[a]));
        ...
        {!loading && data && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <ChartCard title={`Nouveaux utilisateurs (${period}j)`}>
              {(data.curves.usersByDay?.every((r: any) => r.count === 0)) ? (
            ...
            <ChartCard title={`Nouveaux clubs (${period}j)`}>
              {(data.curves.clubsByDay?.every((r: any) => r.count === 0)) ? (
            ...
            <ChartCard title={`Événements créés (${period}j)`}>
              {(data.curves.eventsByDay?.every((r: any) => r.count === 0)) ? (
            ...
            <ChartCard title={`Exports affiches (${period}j)`}>
              {(data.curves.postersByDay?.every((r: any) => r.count === 0)) ? (
              ...
              <ChartCard title="Formats d'affiche utilisés">
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {Object.entries(data.exportByFormat)
                    ...
                    .map(([format, count]) => {
                      const total = Object.values(data.exportByFormat).reduce((s: number, n: unknown) => s + (n as number), 0 as number) as number;
            ...
            {data.topTemplates && data.topTemplates.length > 0 && (
              <ChartCard title="Templates les plus utilisés (tous temps)">
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {data.topTemplates.map((tpl: any, i: number) => {
