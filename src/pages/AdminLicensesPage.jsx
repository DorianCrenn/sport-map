...
const PLAN_COLOR = { starter: '#3b82f6', pro: '#8b5cf6', elite: '#f59e0b' };
const TYPE_LABEL = { manual: 'Manuel', partner: 'Partenaire', trial_extension: 'Essai', lifetime: 'À vie', promo: 'Promo' };
  ...
  return (
    <div style={{ backgroundColor: 'var(--sl-card)', borderRadius: 16, border: `1px solid ${isActive ? color + '40' : 'var(--sl-border)'}`, padding: 14, opacity: isActive ? 1 : 0.65 }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
        ...
        {isActive && !confirm && (
          <button onClick={() => setConfirm(true)} style={{ padding: '5px 10px', borderRadius: 8, border: '1px solid rgba(239,68,68,0.3)', fontSize: 11, fontWeight: 600, color: '#ef4444', backgroundColor: 'rgba(239,68,68,0.08)', cursor: 'pointer', flexShrink: 0 }}>
            Révoquer
          </button>
        )}
        {confirm && (
...
export default function AdminLicensesPage({ onBack }) {
  const { grants, loading, createGrant, revokeGrant } = useAdminGrants();
  const { toast } = useToast();

  const [showForm,    setShowForm]    = useState(false);
  const [filterState, setFilterState] = useState('active'); // active | expired | revoked | all
        ...
        {showForm && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }}
            ...
            <form onSubmit={handleSubmit} style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              {/* Club */}
              <div>
                <p style={{ margin: '0 0 6px', fontSize: 11, color: 'var(--sl-t3)', fontWeight: 600 }}>Club * :</p>
                <input value={clubSearch} onChange={e => { setClubSearch(e.target.value); if (!e.target.value) setFormClubId(''); }}
                  placeholder="Rechercher un club vérifié…"
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                ...
                <div style={{ flex: 1 }}>
                  <p style={{ margin: '0 0 6px', fontSize: 11, color: 'var(--sl-t3)', fontWeight: 600 }}>Durée :</p>
                  <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                    <button type="button" onClick={() => setFormLifetime(p => !p)}
                    ...
                    {!formLifetime && (
                      <input type="number" min="1" max="120" value={formMonths} onChange={e => setFormMonths(Number(e.target.value))}
              ...
              {/* Raison */}
              <div>
                <p style={{ margin: '0 0 6px', fontSize: 11, color: 'var(--sl-t3)', fontWeight: 600 }}>Motif * :</p>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                <button type="submit" disabled={submitting || !formClubId}
