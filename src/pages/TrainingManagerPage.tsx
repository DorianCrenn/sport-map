...
export default function TrainingManagerPage({ onBack }: TrainingManagerPageProps) {
  ...
  const hasGenerated = useRef(new Set<string>());
  useEffect(() => {
    ...
    async function gen() {
      for (const team of clubTeams) {
        const recurring = (trainings[team.id] ?? []).filter((s: any) => s.recurring);
  ...
  const playerGenerated = useRef(new Set<string>());
  useEffect(() => {
    ...
    const recurring = (playerTrainings[playerTeamId] ?? Object.values(playerTrainings).flat()).filter((s: any) => s.recurring);
  ...
  function handleAddTeam() {
    ...
    if (!trainings[key]) setTrainings((prev: any) => ({ ...prev, [key]: [] }));
  ...
  return (
    <motion.div
      ...
      style={{
        ...
        backgroundColor: 'var(--sl-bg)', zIndex: (Z as any).formModal,
      }}
    >
      {/* ── Header ─────────────────────────────────────────────────────── */}
      <div style={{
          ...
          <div style={{ padding: '12px 0 80px' }}>
            ...
            {clubTeams.length > 1 && (
              <div style={{
                ...
                {[{ id: 'all', name: 'Tout' }, ...clubTeams].map((t: any) => (
                  <button
                    key={t.id}
            ...
            {clubsLoading ? (
              ...
              <>
                ...
                <div style={{ flex: 1, overflowY: 'auto', padding: '0 16px 80px', borderTop: '1px solid var(--sl-border)' }}>
                  ...
                  {activeTabId && (() => {
                    const activeTeam = clubTeams.find((t: any) => t.id === activeTabId);
                      ...
                      <div>
                        <div style={{ borderRadius: '0 0 12px 12px', backgroundColor: 'var(--sl-card)', padding: 12, border: '1px solid var(--sl-border)', borderTop: 'none', marginBottom: 12 }}>
                          <TrainingBlock
                            ...
                            isEditing={true}
                            onUpdate={(patch: any) => setTrainings((prev: any) => ({ ...prev, [activeTabId]: patch.sessions }))}
                            clubId={String(selectedClubId)}
                            currentUser={currentUser}
                            isManager={true}
                            allTeams={clubTeams}
                        ...
                        {mergedSessionsForActiveTeam.length > 0 && (
                          <div style={{ marginBottom: 16 }}>
                            ...
                            {mergedSessionsForActiveTeam.map(({ session, hostTeam }: any) => (
...
interface MergedSessionRowProps {
  session: Record<string, any>;
  hostTeam: Record<string, any>;
}

function MergedSessionRow({ session, hostTeam }: MergedSessionRowProps) {
