...
interface CalSvgProps { size?: number; }
...
interface PinSvgProps { size?: number; }
const PinSvg = ({ size = 11 }: PinSvgProps) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0 }}>
    <path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/>
  </svg>
);

interface HeartSvgProps { filled?: boolean; size?: number; }
const HeartSvg = ({ filled, size = 15 }: HeartSvgProps) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill={filled ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
    <path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/>
  </svg>
);

const EV_TYPE_META: Record<string, { label: string; color: string }> = {
...
interface FavoriteCardProps {
  ...
  isAttending?: (id: any) => boolean;
  onToggleAttend?: (id: any) => void;
}
...
function DateGroup({ label, events, onToggleFavorite, accent, isAttending, onToggleAttend }: DateGroupProps) {
  return (
...
interface NotifBannerProps {
  favoriteEvents: Record<string, any>[];
}

function NotifBanner({ favoriteEvents }: NotifBannerProps) {
...
function exportAllICS(events: Record<string, any>[]) {
  function pad(n: number) { return String(n).padStart(2, '0'); }
  function toICS(d: Date) { return `${d.getFullYear()}${pad(d.getMonth() + 1)}${pad(d.getDate())}T${pad(d.getHours())}${pad(d.getMinutes())}00`; }
  function esc(s: any) { return String(s ?? '').replace(/\\/g, '\\\\').replace(/;/g, '\\;').replace(/,/g, '\\,').replace(/\n/g, '\\n'); }
  const now = new Date();
...
interface MatchsTabProps {
  favoriteEvents: Record<string, any>[];
  upcomingFavorites: Record<string, any>[];
  onToggleFavorite: (id: any) => void;
  isAttending?: (id: any) => boolean;
  onToggleAttend?: (id: any) => void;
  onGoToMap?: () => void;
...
export default function MatchsTab({ favoriteEvents, upcomingFavorites, onToggleFavorite, isAttending, onToggleAttend, onGoToMap }: MatchsTabProps) {
  const groups = useMemo(() => groupByDate(favoriteEvents as any), [favoriteEvents]) as any;
