...
// Palette de couleurs proposées pour les plans
const COLOR_PALETTE = [
...
function TarifTab({ planMeta, planOrder, updatePricing, toast }) {
  const [editingPlan, setEditingPlan] = useState(null); // plan_id en cours d'édition
...
export default function AdminPlansPage({ onBack }) {
  ...
  useEffect(() => {
    Promise.all(
      ...
      supabase.from('clubs').select('id', { count: 'exact', head: true })
        .eq('status', 'verified')
        .then(({ count: total }) => {
          const withSub = Object.values(stats).reduce((s, n) => s + n, 0);
  ...
  async function handleFeatureSave(featureKey, newPlan) {
    setSaving(true);
    try {
  ...
  async function handleQuotaSave(plan, quotaKey, newValue) {
    setSaving(true);
    try {
  ...
  // Plans pour les lignes du tableau (depuis planMeta)
                          ...
                          <td key={p.id} style={{ textAlign: 'center', padding: '8px 10px' }}>
                            {isEditing ? (
                              <input
                                ...
                                onKeyDown={e => { if (e.key === 'Enter') e.target.blur(); if (e.key === 'Escape') setEditingQuota(null); }}
                                autoFocus
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {plansForTable.map(plan => {
              const planFeatures = features?.filter(f => {
                const planRanks = { free: 0, starter: 1, pro: 2, elite: 3 };
                  ...
                  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 10 }}>
                    {Object.entries(planQuotaRows).map(([key, q]) => (
                      <span key={key} style={{ fontSize: 11, padding: '3px 8px', borderRadius: 20, backgroundColor: 'var(--sl-surface)', color: 'var(--sl-t2)' }}>
                        {QUOTA_LABELS_DISPLAY[key]?.label ?? key} : {q.value === null ? '∞' : q.value === 0 ? '—' : q.value}
                      </span>
                    ))}
                  </div>
