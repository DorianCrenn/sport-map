...
export default function HelpPage({ onClose, onOpenFeedback, notifications = [], unreadCount = 0, onMarkRead, onMarkAllRead }) {
  ...
  const { fetchIdeas, vote, unvote, ideas } = useFeedback();
  ...
  useEffect(() => {
    if (tab === 'ideas' && ideas.length === 0) {
      setIdeasLoading(true);
      fetchIdeas().finally(() => setIdeasLoading(false));
    }
  }, [tab, ideas.length, fetchIdeas]);

  const persistVotes = useCallback((set) => {
