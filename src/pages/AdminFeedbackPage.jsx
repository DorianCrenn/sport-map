...
// ── Panel détail feedback ─────────────────────────────────────────────────────

function FeedbackDetail({ item, onClose, onSave }) {
    ...
    >
      ...
      <div style={{ flex: 1, padding: '16px', display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        {!showMerge ? (
          ...
          <div style={{ backgroundColor: 'var(--sl-card)', borderRadius: 14, padding: 14, border: '1px solid var(--sl-border)' }}>
            <p style={{ margin: '0 0 8px', fontSize: 12, color: 'var(--sl-t3)' }}>
              Fermer ce feedback et le marquer comme doublon de l'entrée #ID :
            </p>
            <button
              onClick={() => setShowMerge(false)}
              style={{ fontSize: 11, color: 'var(--sl-t3)', background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}
            >
              Annuler
            </button>
          </div>
    ...
    >
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {item.description && (
            <p style={{ margin: '0 0 6px', fontSize: 12, color: 'var(--sl-t3)', overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>
              {item.description}
            </p>
          )}
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
