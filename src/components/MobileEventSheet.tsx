...
export default function MobileEventSheet({ event, club, onClose, onEdit, onDelete, onUpdateEvent }: MobileEventSheetProps) {
  ...
  const SNAP_H: Record<SnapPoint, string> = { peek: '108px', detail: '72dvh', full: '100%' };

  const group = (SPORTS[event.sport] as any);
  const sportColor: string = group?.color ?? '#22d96a';
  ...
  async function handleShare() {
    ...
    const result = await (share as any)({ title: event.title, text: lines.join('\n'), url: eventUrl });
  ...
  return (
    <>
    <motion.div ref={sheetRef} role="dialog" aria-modal="true" aria-label="Détails de l'événement" drag={snapPoint === 'full' ? false : 'y'} dragConstraints={{ top: 0, bottom: 0 }} dragElastic={{ top: 0.2, bottom: 0.4 }} onDragEnd={handleDragEnd} initial={{ y: '100%' }} animate={{ y: 0, height: SNAP_H[snapPoint] }} exit={{ y: '100%' }} transition={{ type: 'spring', stiffness: 380, damping: 40 }} style={{ position: 'absolute', bottom: 0, left: 0, right: 0, borderRadius: '22px 22px 0 0', backgroundColor: 'var(--sl-card)', zIndex: 'var(--sl-z-sheet)', boxShadow: '0 -8px 40px rgba(0,0,0,0.5)', border: '1px solid var(--sl-border)', borderBottom: 'none', display: 'flex', flexDirection: 'column', overflow: 'hidden', touchAction: snapPoint === 'full' ? 'auto' : 'none' }}>
      {/* Sticky header */}
      ...
      {/* Scrollable content */}
      <div style={{ flex: 1, overflowY: snapPoint === 'full' ? 'auto' : 'hidden', padding: '16px 16px calc(32px + env(safe-area-inset-bottom, 0px))', WebkitOverflowScrolling: 'touch' as any, overscrollBehavior: 'contain' }}>
        {/* Top row */}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 16 }}>
          ...
          {(event.venue || event.city) && (
            <MetaRow icon={<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>}>
              <div style={{ fontSize: 13, color: 'var(--sl-t1)', fontWeight: 500 }}>{[event.venue, event.city].filter(Boolean).join(' · ')}</div>
            </MetaRow>
          )}
        </div>
        ...
        {!isPast && <EventPredictions eventId={event.id} event={event} />}
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }}>
              ...
              {event.standings && (
                <div style={{ borderTop: '1px solid var(--sl-border)', paddingTop: 14, marginBottom: 14 }}>
                  ...
                  {[event.standings.home, event.standings.away].map((team: any) => {
              ...
              {canEditThis && isPast && onUpdateEvent && <ScoreEntryContainer event={event} onUpdateEvent={onUpdateEvent} />}

              <EventPhotoGallery eventId={event.id} clubId={event.clubId} currentUserId={currentUser?.id} canUpload={canEditThis && isPast} />
