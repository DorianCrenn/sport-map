...
function PhotoCard({ photo, canDelete, onDelete, onUseBg }: PhotoCardProps) {
  ...
  return (
    <div style={{ position: 'relative', aspectRatio: '4/3', borderRadius: 10, overflow: 'hidden', backgroundColor: 'var(--sl-surface)', cursor: 'pointer', flexShrink: 0, width: 140 }} onClick={() => setShowActions(v => !v)}>
      ...
      {showActions && (
        <div style={{ position: 'absolute', inset: 0, backgroundColor: 'rgba(0,0,0,0.6)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 8, padding: 8 }}>
          {onUseBg && <button onClick={e => { e.stopPropagation(); onUseBg(photo.url); setShowActions(false); }} style={{ width: '100%', padding: '7px 0', borderRadius: 8, border: 'none', cursor: 'pointer', backgroundColor: '#3b82f6', color: '#fff', fontSize: 11, fontWeight: 700 }}>Utiliser comme fond</button>}
          {canDelete && <button onClick={e => { e.stopPropagation(); onDelete(photo.id); }} style={{ width: '100%', padding: '7px 0', borderRadius: 8, border: 'none', cursor: 'pointer', backgroundColor: 'rgba(239,68,68,0.2)', color: '#ef4444', fontSize: 11, fontWeight: 700 }}>Supprimer</button>}
          <button onClick={e => { e.stopPropagation(); setShowActions(false); }} style={{ width: '100%', padding: '7px 0', borderRadius: 8, border: 'none', cursor: 'pointer', backgroundColor: 'rgba(255,255,255,0.1)', color: '#fff', fontSize: 11 }}>Annuler</button>
        </div>
      )}
    </div>
  );
    ...
    <div style={{ marginTop: 12 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
        ...
        {canUpload && (photos as Photo[]).length < (maxPhotos as number) && (
          <button onClick={() => inputRef.current?.click()} disabled={uploading as boolean} style={{ fontSize: 11, fontWeight: 700, padding: '4px 10px', borderRadius: 8, border: 'none', cursor: (uploading as boolean) ? 'default' : 'pointer', backgroundColor: 'var(--sl-surface)', color: 'var(--sl-t2)', opacity: (uploading as boolean) ? 0.5 : 1 }}>{uploading ? 'Envoi…' : '+ Photo'}</button>
        )}
      </div>
      ...
      {(photos as Photo[]).length === 0 && canUpload && (
        <button onClick={() => inputRef.current?.click()} disabled={uploading as boolean} style={{ width: '100%', padding: '16px 0', borderRadius: 12, border: '1.5px dashed var(--sl-border)', backgroundColor: 'transparent', cursor: 'pointer', fontSize: 12, color: 'var(--sl-t3)' }}>{uploading ? 'Envoi en cours…' : 'Ajouter des photos post-match'}</button>
      )}
    </div>
  );
}
