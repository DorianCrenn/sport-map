...
interface EventItem { id: string | number; title: string; sport: string; city?: string | null; date: string; }
...
interface SearchResult { type: 'city' | 'club' | 'event'; key: string; data: any; }
...
export default function Header({
  ...
  const allResults: SearchResult[] = useMemo(() => [
    ...matchedCities.map(c => ({ type: 'city' as const, key: c, data: c })),
    ...matchedClubs.map(c => ({ type: 'club' as const, key: String(c.id), data: c })),
    ...matchedEvents.map(e => ({ type: 'event' as const, key: String(e.id), data: e })),
  ...
  function selectCity(city: string) { onCityFilter?.(city); setQuery(''); setSearchOpen(false); }
  function selectClub(club: Club) { onSelectClub?.(club); setQuery(''); setSearchOpen(false); }
  function selectEvent(event: EventItem) { onSelectEvent?.(event); setQuery(''); setSearchOpen(false); }
      ...
      <div style={{ flex: 1, minWidth: 0 }} ref={searchRef}>
        {cityFilter ? (
          ...
          <div style={{ position: 'relative' }}>
            <svg style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', pointerEvents: 'none', transition: 'opacity 0.2s' }} width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={inputFocused ? 'rgba(34,217,106,0.8)' : 'rgba(222,238,255,0.35)'} strokeWidth="2.5" strokeLinecap="round">
            ...
            {query && (
              <button onClick={() => { setQuery(''); setSearchOpen(false); inputRef.current?.focus(); }} aria-label="Effacer la recherche" style={{ position: 'absolute', right: 4, top: '50%', transform: 'translateY(-50%)', width: 44, height: 44, borderRadius: '50%', backgroundColor: 'transparent', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <div style={{ width: 20, height: 20, borderRadius: '50%', backgroundColor: 'rgba(255,255,255,0.14)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <svg width="8" height="8" viewBox="0 0 24 24" fill="none" stroke="rgba(222,238,255,0.8)" strokeWidth="3" strokeLinecap="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
                </div>
              </button>
            )}
          </div>
            ...
            <motion.div key="dropdown" role="listbox" aria-label="Résultats de recherche" initial={{ opacity: 0, y: -8, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -6, scale: 0.98 }} transition={{ duration: 0.14, ease: [0.16, 1, 0.3, 1] }} style={{ position: 'absolute', top: 'calc(100% + 6px)', left: 12, right: 12, borderRadius: 18, backgroundColor: 'var(--sl-card)', boxShadow: 'var(--sl-shadow-xl)', border: '1px solid var(--sl-border)', zIndex: 200, overflow: 'hidden' }}>
              {showEmpty && (
                <div style={{ padding: '22px 16px', textAlign: 'center' }}>
                  ...
                  <p style={{ fontSize: 13, fontWeight: 600, color: 'var(--sl-t2)', margin: 0 }}>Aucun résultat pour « {query.trim()} »</p>
                  <p style={{ fontSize: 11, color: 'var(--sl-t3)', marginTop: 4 }}>Essayez un sport, une ville ou un club différent</p>
                </div>
              )}
              ...
              {matchedCities.length > 0 && (matchedClubs.length > 0 || matchedEvents.length > 0) && <Divider />}
              ...
              {matchedClubs.length > 0 && matchedEvents.length > 0 && <Divider />}
              ...
              {hasResults && (
                <div style={{ padding: '6px 14px 10px', borderTop: '1px solid var(--sl-border)', display: 'flex', alignItems: 'center', gap: 6 }}>
                  <Kbd>↑↓</Kbd><span style={{ fontSize: 10, color: 'var(--sl-t3)' }}>naviguer</span>
                  <Kbd>↵</Kbd><span style={{ fontSize: 10, color: 'var(--sl-t3)' }}>sélectionner</span>
                  <Kbd>Esc</Kbd><span style={{ fontSize: 10, color: 'var(--sl-t3)' }}>fermer</span>
                </div>
              )}
            </motion.div>
          )}
              ...
              <div style={{ padding: '14px 14px 10px', borderBottom: '1px solid var(--sl-border)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <div style={{ minWidth: 0 }}>
                    <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--sl-t1)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{currentUser.name}</div>
                    <div style={{ fontSize: 11, color: 'var(--sl-t3)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{currentUser.email}</div>
                  </div>
                </div>
              </div>
              <div style={{ padding: '6px' }}>
                <DropdownItem icon={<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>} label="Mon profil" onClick={() => { setProfileOpen(false); onTabChange?.('profil'); }} />
                {onMyRides && <DropdownItem icon={<span style={{ fontSize: 14 }}>🚗</span>} label="Mes covoiturages" badge={rideNotifCount} onClick={() => { setProfileOpen(false); onMyRides(); }} />}
                {isAdmin && <DropdownItem icon={<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/></svg>} label={clubNotifUnread > 0 ? `Dashboard admin · ${clubNotifUnread} nouveau${clubNotifUnread > 1 ? 'x' : ''}` : 'Dashboard admin'} labelColor={clubNotifUnread > 0 ? '#f59e0b' : undefined} onClick={() => { setProfileOpen(false); onTabChange?.('admin'); }} />}
                {isClubAdmin && <DropdownItem icon={<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>} label="Mon club" onClick={() => { setProfileOpen(false); onTabChange?.('clubs'); }} />}
                <DropdownItem icon={theme === 'dark' ? <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><circle cx="12" cy="12" r="5"/><path d="M12 1v2M12 21v2M4.22 4.22l1.42 1.42M18.36 18.36l1.42 1.42M1 12h2M21 12h2M4.22 19.78l1.42-1.42M18.36 5.64l1.42-1.42"/></svg> : <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"/></svg>} label={theme === 'dark' ? 'Mode clair' : 'Mode sombre'} onClick={() => { toggleTheme(); setProfileOpen(false); }} />
              </div>
              <div style={{ height: 1, backgroundColor: 'var(--sl-border)', margin: '0 6px' }} />
              <div style={{ padding: '6px' }}>
                <DropdownItem icon={<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#ef4444" strokeWidth="2" strokeLinecap="round"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" y1="12" x2="9" y2="12"/></svg>} label="Déconnexion" labelColor="#ef4444" onClick={() => { logout(); setProfileOpen(false); }} />
              </div>
            </motion.div>
          )}
        </AnimatePresence>
...
function SectionLabel({ label }: { label: string }) {
  return (
    <div style={{ padding: '10px 14px 5px', display: 'flex', alignItems: 'center', gap: 6 }}>
      <span style={{ fontSize: 10, fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.1em', color: 'var(--sl-t3)' }}>{label}</span>
    </div>
  );
}
...
interface DropdownItemProps { icon?: React.ReactNode; label: string; onClick: () => void; labelColor?: string; badge?: number; }
