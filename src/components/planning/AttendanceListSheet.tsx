...
function Section({ status, records }: { status: string; records: AttendanceRecord[] }) {
...
interface SheetContentProps {
  ...
  byStatus: Record<string, AttendanceRecord[]>;
  onClose: () => void;
}

function SheetContent({ title, counts, byStatus, onClose }: SheetContentProps) {
  return (
    <>
...
function TrainingSheet({ sessionId, userId, onClose }: { sessionId: string | number; userId?: string | null; onClose: () => void }) {
  const { attendance, counts } = useTrainingAttendance(String(sessionId), userId) as { attendance: AttendanceRecord[]; counts: AttendanceCounts };
  const byStatus = {
...
function MatchSheet({ eventId, userId, onClose }: { eventId: string | number; userId?: string | null; onClose: () => void }) {
  const { attendance, counts } = useMatchAttendance(String(eventId), userId) as { attendance: AttendanceRecord[]; counts: AttendanceCounts };
  const byStatus = {
    present: attendance.filter(a => a.status === 'present'),
    absent:  attendance.filter(a => a.status === 'absent'),
    unsure:  attendance.filter(a => a.status === 'unsure'),
...
export default function AttendanceListSheet({ open, onClose, type, id, userId }: AttendanceListSheetProps) {
  useEffect(() => {
    if (!open) return;
