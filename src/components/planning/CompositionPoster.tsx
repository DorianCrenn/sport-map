...
const STATUS_FILTER: Record<string, { label: string; include: string[] }> = {
...
interface Convocation {
  id?: string | number;
  status: string;
  player?: { name?: string | null; photo_url?: string | null; number?: string | number | null } | null;
  player_name?: string | null;
  photo_url?: string | null;
  number?: string | number | null;
}

function PlayerCard({ player, status }: { player: Convocation; status: string }) {
  ...
  return (
    <div className="flex flex-col items-center gap-1" style={{ opacity: isConfirmed ? 1 : 0.65 }}>
      <div className="relative rounded-xl overflow-hidden flex items-center justify-center"
        style={{ width: 64, height: 64, background: isConfirmed ? 'rgba(34,217,106,0.15)' : 'rgba(245,158,11,0.12)', border: `2px ${isConfirmed ? 'solid' : 'dashed'} ${isConfirmed ? '#22d96a' : '#f59e0b'}` }}
      >
        {photoUrl
          ? <img src={photoUrl} alt={name} className="w-full h-full object-cover" />
          : <span className="text-lg font-black" style={{ color: isConfirmed ? '#22d96a' : '#f59e0b' }}>{initials}</span>
        }
        <div className="absolute bottom-0.5 right-0.5 w-4 h-4 rounded-full flex items-center justify-center text-[8px] font-black" style={{ background: isConfirmed ? '#22d96a' : '#f59e0b', color: '#000' }}>
          {isConfirmed ? '✓' : '⏳'}
        </div>
      </div>
      <span className="text-[9px] font-bold text-center leading-tight" style={{ color: isConfirmed ? '#e2e8f0' : '#94a3b8', maxWidth: 64, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' } as React.CSSProperties}>
        {name}
      </span>
      {number && (
        <span className="text-[8px] font-black" style={{ color: isConfirmed ? '#22d96a' : '#f59e0b' }}>#{number}</span>
      )}
    </div>
  );
}

interface CompositionPosterProps {
  event?: Record<string, any> | null;
  club?: Record<string, any> | null;
  onClose: () => void;
}

export default function CompositionPoster({ event, club, onClose }: CompositionPosterProps) {
  const { convocations, loading } = useEventConvocations(event?.id) as { convocations: Convocation[]; loading: boolean };
  ...
  const displayedPlayers = convocations.filter(c => STATUS_FILTER[showFilter].include.includes(c.status));
  ...
  return (
    <AnimatePresence>
      <motion.div className="fixed inset-0 z-50 flex flex-col bg-black/70 overscroll-contain" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
        <div className="flex items-center justify-between px-4 py-3 bg-black/80 backdrop-blur-sm flex-shrink-0">
          <button onClick={onClose} className="text-white/70 text-sm font-semibold hover:text-white transition-colors">✕ Fermer</button>
          <h2 className="text-white font-bold text-sm">Affiche du groupe</h2>
          <button onClick={handleExport} disabled={exporting || ordered.length === 0} className="text-xs font-bold px-3 py-1.5 rounded-lg text-black transition-opacity disabled:opacity-40" style={{ background: '#22d96a' }}>
            {exporting ? '…' : '⬇ Exporter'}
          </button>
        </div>
        ...
        <div className="flex justify-center gap-2 py-2 bg-black/60 flex-shrink-0">
          {Object.entries(STATUS_FILTER).map(([key, cfg]) => (
            <button key={key} onClick={() => setShowFilter(key)} className="text-[11px] font-bold px-3 py-1.5 rounded-full transition-all"
              style={showFilter === key ? { background: '#22d96a', color: '#000' } : { background: 'rgba(255,255,255,0.1)', color: 'rgba(255,255,255,0.7)' }}
            >{cfg.label}</button>
          ))}
        </div>
          ...
          {loading ? (
            ...
            <div ref={posterRef} style={{ width: 360, minHeight: 480, background: 'linear-gradient(160deg, #0f172a 0%, #1e293b 60%, #0f172a 100%)', borderRadius: 20, overflow: 'hidden', fontFamily: "'Inter', sans-serif", flexShrink: 0 }}>
              <div style={{ background: `linear-gradient(135deg, ${clubPrimary}dd, ${clubPrimary}88)`, padding: '20px 16px 16px' }}>
                ...
                {matchDate && <div style={{ color: 'rgba(255,255,255,0.75)', fontSize: 11, marginTop: 4 }}>{matchDate}{matchTime ? ` · ${matchTime}` : ''}</div>}
                {event?.location && <div style={{ color: 'rgba(255,255,255,0.6)', fontSize: 10, marginTop: 2 }}>📍 {event.location}</div>}
              </div>
              ...
              {ordered.length > 0 ? (
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10, padding: '12px 16px 20px' }}>
                  {ordered.map((p, i) => <PlayerCard key={p.id ?? i} player={p} status={p.status} />)}
                </div>
              ) : (
                <div style={{ textAlign: 'center', color: 'rgba(255,255,255,0.4)', fontSize: 12, padding: '32px 16px' }}>Aucun joueur à afficher</div>
              )}
