...
const STEPS_BY_ROLE: Record<string, Step[]> = {
  ...
  communicant: [{ id: 'poster',         emoji: '🎨', label: 'Créer une affiche',              sublabel: 'PosterStudio — 40 templates professionnels',          tab: 'map'    }, { id: 'club_page',      emoji: '🏟️', label: 'Personnaliser la page club',  sublabel: 'Mini-site public de votre club',                      tab: 'clubs'  }, { id: 'send_announce',  emoji: '📢', label: 'Envoyer une annonce',         sublabel: 'Urgent, info, résultat, événement',                   tab: 'clubs'  }, { id: 'follow_club',    emoji: '❤️', label: 'Suivre votre club',           sublabel: 'Accès aux actualités dans le fil Accueil',            tab: 'clubs'  }],
  parent:      [{ id: 'join_team',     emoji: '🔗', label: "Rejoindre l'équipe de votre enfant", sublabel: 'Trouvez le club et rattachez sa fiche pour recevoir les convocations', tab: 'clubs' }, { id: 'follow_club',    emoji: '❤️', label: "Suivre le club de votre enfant", sublabel: 'Annonces et matchs directement dans votre fil',       tab: 'clubs'  }, { id: 'convocations',   emoji: '📋', label: 'Voir les convocations',       sublabel: 'Répondez aux sélections de votre enfant',            tab: 'home'   }, { id: 'notifications',  emoji: '🔔', label: 'Activer les notifications',   sublabel: 'Ne manquez aucun match ni annonce',                   tab: 'profil' }],
  joueur:      [{ id: 'join_team',     emoji: '🔗', label: 'Rejoindre mon équipe',           sublabel: 'Trouvez votre club et rattachez votre fiche pour vos convocations', tab: 'clubs' }, { id: 'follow_club',    emoji: '❤️', label: 'Suivre votre club',              sublabel: 'Actualités, résultats, annonces',                     tab: 'clubs'  }, { id: 'convocations',   emoji: '📋', label: 'Voir mes convocations',       sublabel: 'Répondez aux sélections de votre coach',             tab: 'home'   }, { id: 'notifications',  emoji: '🔔', label: 'Activer les notifications',   sublabel: 'Rappels de matchs, nouvelles convocations',           tab: 'profil' }],
...
export default function OnboardingFirstSteps({ jobRole, onNavigate, onClose }: OnboardingFirstStepsProps) {
  ...
  function handleAction(step: Step) {
    ...
    // Intent : "Créer votre club" ouvre directement le wizard (voir ClubsPage),
    // au lieu de juste déposer le coach sur l'onglet Clubs.
    if (step.id === 'create_club') sessionStorage.setItem('sl-open-club-wizard', '1');
    onNavigate?.(step.tab);
  }
