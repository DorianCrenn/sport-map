  ...
  return (
    <motion.div role="dialog" aria-modal="true" aria-label="Profil membre" initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: 'spring', stiffness: 340, damping: 36 }} style={{ position: 'absolute', inset: 0, zIndex: 'var(--sl-z-sheet)', display: 'flex', flexDirection: 'column', backgroundColor: 'var(--sl-bg)', fontFamily: '"Inter", "Helvetica Neue", Arial, sans-serif', overflowY: 'auto' }}>
      <div style={{ flexShrink: 0, padding: '14px 16px', backgroundColor: 'var(--sl-card)', borderBottom: '1px solid var(--sl-border)', display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <button onClick={handleShare} style={{ width: 44, height: 44, borderRadius: 'var(--sl-radius-lg)', border: 'none', cursor: 'pointer', backgroundColor: copied ? 'rgba(34,217,106,0.12)' : 'var(--sl-surface)', color: copied ? 'var(--sl-green)' : 'var(--sl-t2)', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.2s' }} title="Partager ce profil">
      ...
      {loading ? (
        ...
        <div style={{ flex: 1, padding: '20px 16px calc(32px + env(safe-area-inset-bottom, 0px))', display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div style={{ borderRadius: 'var(--sl-radius-4xl)', padding: '20px', backgroundColor: 'var(--sl-card)', border: '1px solid var(--sl-border)', display: 'flex', alignItems: 'center', gap: 16 }}>
          ...
          <div style={{ borderRadius: 'var(--sl-radius-4xl)', padding: '18px', backgroundColor: 'var(--sl-card)', border: '1px solid var(--sl-border)' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
              ...
              <div style={{ width: 44, height: 44, borderRadius: 'var(--sl-radius-xl)', background: 'linear-gradient(135deg, var(--sl-green), #16a34a)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, fontWeight: 800, color: '#fff', flexShrink: 0 }}>{levelInfo.level}</div>
            </div>
            {levelInfo.nextLevel && <div style={{ height: 6, borderRadius: 'var(--sl-radius-xs)', backgroundColor: 'var(--sl-border)', overflow: 'hidden' }}><div style={{ height: '100%', borderRadius: 'var(--sl-radius-xs)', background: 'linear-gradient(90deg, var(--sl-green), #16a34a)', width: `${Math.round(levelInfo.progress * 100)}%`, transition: 'width 0.6s ease' }} /></div>}
          </div>
          {favSports.length > 0 && (
            <div style={{ borderRadius: 'var(--sl-radius-4xl)', padding: '18px', backgroundColor: 'var(--sl-card)', border: '1px solid var(--sl-border)' }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                {favSports.map(sport => (
                  <div key={sport} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 12px', borderRadius: 'var(--sl-radius-4xl)', backgroundColor: 'var(--sl-surface)', border: '1px solid var(--sl-border)' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {followedClubs.map(club => (
                  <div key={club.id} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <div style={{ width: 36, height: 36, borderRadius: 'var(--sl-radius-lg)', overflow: 'hidden', backgroundColor: 'var(--sl-surface)', border: '1px solid var(--sl-border)', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <div style={{ borderRadius: 'var(--sl-radius-4xl)', padding: '18px', backgroundColor: 'var(--sl-card)', border: '1px solid var(--sl-border)' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8 }}>
              {(BADGE_ORDER as string[]).map(id => {
                ...
                return (
                  <div key={id} title={`${def.name} — ${def.description}`} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, padding: '10px 4px', borderRadius: 'var(--sl-radius-xl)', backgroundColor: earned ? `${def.color}12` : 'var(--sl-surface)', border: `1px solid ${earned ? def.color + '30' : 'var(--sl-border)'}`, opacity: earned ? 1 : 0.4, transition: 'all 0.2s' }}>
