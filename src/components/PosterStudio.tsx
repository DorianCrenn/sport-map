...
export default function PosterStudio({ event, onClose, club, quickMode = false, resultMode = null, initialBgSrc = null, convocationPlayers = null }) {
  ...
  } = usePosterAssets({ clubId: club?.id, accentColor, dispatch, overlayElements: overlayElements as any, aiOverlayElements: aiOverlayElements as any, playerLayers: playerLayers as any });
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, flex: 1, minWidth: 0, overflow: 'hidden' }}>
            ...
            <div style={{ minWidth: 0, overflow: 'hidden' }}>
              ...
              <div style={{ fontSize: 10, color: 'var(--sl-t3)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{event?.title}</div>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0, marginLeft: 8 }}>
            ...
            <AnimatePresence>
              {isDesktop && restoredDraft && (
                <motion.span key="restored" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                  style={{ fontSize: 10, color: '#3b82f6', fontWeight: 700, whiteSpace: 'nowrap' }}>Brouillon restauré</motion.span>
              )}
              {isDesktop && !restoredDraft && lastSavedAt && (
                <motion.span key="saved" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                  style={{ fontSize: 10, color: 'var(--sl-green)', fontWeight: 600, display: 'flex', alignItems: 'center', gap: 3, whiteSpace: 'nowrap' }}>
              ...
              {/* Indicateur sauvegarde mobile : juste la coche */}
              {!isDesktop && !restoredDraft && lastSavedAt && (
                <motion.span key="saved-mobile" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                  style={{ color: 'var(--sl-green)', display: 'flex', alignItems: 'center' }}>
