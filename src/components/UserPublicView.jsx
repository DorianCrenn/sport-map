...
function InitialAvatar({ userId, size = 56 }) {
...
export default function UserPublicView({ userId, onClose }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      supabase
        .from('profiles')
        .select('id, xp, favorite_sports, badges, role, created_at')
        .eq('id', userId)
        .maybeSingle(),
      supabase
        .from('club_follows')
        .select('club_id, clubs(id, name, sport, logo_url)')
        .eq('user_id', userId)
        .limit(12),
    ]).then(([{ data: prof }, { data: follows }]) => {
      setProfile(prof);
      setFollowedClubs((follows ?? []).map(f => f.clubs).filter(Boolean));
      setLoading(false);
    });
  }, [userId]);
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        <h1 style={{ fontSize: 17, fontWeight: 800, color: 'var(--sl-t1)', margin: 0, letterSpacing: '-0.02em', flex: 1 }}>
          Profil membre
        </h1>
        <button
          onClick={handleShare}
        ...
        <div style={{ flex: 1, padding: '20px 16px calc(32px + env(safe-area-inset-bottom, 0px))', display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ borderRadius: 20, padding: '20px', backgroundColor: 'var(--sl-card)', border: '1px solid var(--sl-border)', display: 'flex', alignItems: 'center', gap: 16 }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 17, fontWeight: 800, color: 'var(--sl-t1)', letterSpacing: '-0.02em', marginBottom: 4 }}>
                Membre SportLink
              </div>
              {memberSince && (
                <div style={{ fontSize: 11, color: 'var(--sl-t3)' }}>Membre depuis {memberSince}</div>
              )}
            </div>
          </div>
          ...
          <div style={{ borderRadius: 20, padding: '18px', backgroundColor: 'var(--sl-card)', border: '1px solid var(--sl-border)' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
              <div>
                ...
                <div style={{ fontSize: 11, color: 'var(--sl-t3)', marginTop: 2 }}>
                  {xp} XP{levelInfo.nextLevel ? ` · ${levelInfo.nextLevel.minXp - xp} XP pour ${levelInfo.nextLevel.name}` : ' · Niveau max'}
                </div>
              </div>
              <div style={{
                width: 44, height: 44, borderRadius: 12,
                background: 'linear-gradient(135deg, var(--sl-green), #16a34a)',
                display: 'flex', alignItems: 'center', justifyContent: 'center',
            ...
            {levelInfo.nextLevel && (
              <div style={{ height: 6, borderRadius: 3, backgroundColor: 'var(--sl-border)', overflow: 'hidden' }}>
                <div style={{ height: '100%', borderRadius: 3, background: 'linear-gradient(90deg, var(--sl-green), #16a34a)', width: `${Math.round(levelInfo.progress * 100)}%`, transition: 'width 0.6s ease' }} />
              </div>
            )}
          </div>
          ...
          {favSports.length > 0 && (
            <div style={{ borderRadius: 20, padding: '18px', backgroundColor: 'var(--sl-card)', border: '1px solid var(--sl-border)' }}>
              <div style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', color: 'var(--sl-t3)', marginBottom: 10 }}>
                Sports pratiqués
              </div>
          ...
          {followedClubs.length > 0 && (
            <div style={{ borderRadius: 20, padding: '18px', backgroundColor: 'var(--sl-card)', border: '1px solid var(--sl-border)' }}>
              <div style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', color: 'var(--sl-t3)', marginBottom: 10 }}>
                Clubs suivis ({followedClubs.length})
              </div>
                  ...
                  <div key={club.id} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <div style={{ width: 36, height: 36, borderRadius: 10, overflow: 'hidden', backgroundColor: 'var(--sl-surface)', border: '1px solid var(--sl-border)', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      {club.logo_url
                        ...
                        : <SportIcon sport={club.sport} size={18} />}
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
