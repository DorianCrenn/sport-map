...
function QuickScoreEdit({ event, onUpdateEvent, onPosterResult, onAnnounceResult }) {
  ...
  useEffect(() => {
    // Synchronise l'état local avec les props externes (score mis à jour ailleurs)
          ...
          <div style={{
            ...
            {filteredPlayers.slice(0, 8).map(p => (
              <button key={p.id} type="button"
                ...
                style={{ display: 'flex', alignItems: 'center', gap: 8, width: '100%', padding: '11px 12px', fontSize: 12, textAlign: 'left', backgroundColor: 'transparent', border: 'none', cursor: 'pointer', color: 'var(--sl-text)', minHeight: 44 }}
...
function TournamentCardContent({ event, isSelected, canEditThis, onEdit, onDelete, onDuplicate, setShowPoster, attendeeCount, status }) {
