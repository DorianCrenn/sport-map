...
export default function FollowModal({ club, allEvents = [], currentFollow = null, onSave, onClose }) {
  ...
  const availableTeams = useMemo(() => {
    const names = new Set();
    allEvents
      .filter(e => String(e.clubId) === String(club.id) && e.teamName)
      .forEach(e => names.add(e.teamName));
    (club.categories ?? [])
      .flatMap(c => c.teams ?? [])
  ...
  const [followAll, setFollowAll] = useState(() =>
    currentFollow ? currentFollow.teams === 'all' : true
  );
  const [selectedTeams, setSelectedTeams] = useState(() => {
    if (!currentFollow || currentFollow.teams === 'all') return new Set(availableTeams);
    return new Set(currentFollow.teams);
  });
  ...
  function handleSave() {
    const teams = followAll || availableTeams.length === 0 ? 'all' : [...selectedTeams];
    onSave({ teams, notif });
  }
      ...
      >
        ...
        >
          ...
          <div style={{ overflowY: 'auto', padding: '16px 20px 24px' }}>
            ...
            {/* Follow scope — only shown if there are teams */}
            {availableTeams.length > 0 && (
              <>
                <div style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: 'var(--sl-t3)', marginBottom: 10 }}>
                  Que souhaitez-vous suivre ?
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 20 }}>
                  {[
                    { val: true,  title: 'Tout le club',        sub: 'Tous les matchs et équipes' },
                    { val: false, title: 'Certaines équipes',   sub: 'Choisir les équipes à suivre' },
                  ].map(({ val, title, sub }) => (
                    <button
                      key={String(val)}
                      onClick={() => setFollowAll(val)}
                      style={{
                        display: 'flex', alignItems: 'center', gap: 13,
                        padding: '12px 14px', borderRadius: 13, cursor: 'pointer', textAlign: 'left',
                        border: `1.5px solid ${followAll === val ? sportColor : 'var(--sl-border)'}`,
                        backgroundColor: followAll === val ? `${sportColor}12` : 'var(--sl-surface)',
                        transition: 'all 0.15s',
                      }}
                    >
                      <div style={{
                        width: 18, height: 18, borderRadius: '50%', flexShrink: 0,
                        border: `2px solid ${followAll === val ? sportColor : 'var(--sl-border-s)'}`,
                        display: 'flex', alignItems: 'center', justifyContent: 'center',
                        transition: 'border-color 0.15s',
                      }}>
                        {followAll === val && <div style={{ width: 8, height: 8, borderRadius: '50%', backgroundColor: sportColor }} />}
                      </div>
                      <div>
                        <div style={{ fontWeight: 600, fontSize: 13, color: 'var(--sl-t1)' }}>{title}</div>
                        <div style={{ fontSize: 11, color: 'var(--sl-t3)', marginTop: 1 }}>{sub}</div>
                      </div>
                    </button>
                  ))}
                </div>
                    ...
                    >
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                        {availableTeams.map(teamName => {
                          const checked = selectedTeams.has(teamName);
                          return (
                            <button
                              key={teamName}
                              ...
                              style={{
                                ...
                                border: `1px solid ${checked ? sportColor : 'var(--sl-border)'}`,
                                backgroundColor: checked ? `${sportColor}10` : 'var(--sl-surface)',
                                transition: 'all 0.12s',
                              }}
                            >
                              <div style={{
                                width: 18, height: 18, borderRadius: 5, flexShrink: 0,
                                border: `2px solid ${checked ? sportColor : 'var(--sl-border-s)'}`,
            ...
            <div style={{ marginBottom: 24 }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                {[
                  { key: 'match', icon: '🏆', label: 'Matchs',    sub: '1h avant chaque match' },
                  { key: 'news',  icon: '📰', label: 'Événements', sub: 'Nouveaux événements' },
                ].map(({ key, icon, label, sub }) => (
                  <button
                    key={key}
                    onClick={() => setNotif(p => ({ ...p, [key]: !p[key] }))}
                    style={{
                      flex: 1, padding: '11px 12px', borderRadius: 12, cursor: 'pointer', textAlign: 'left',
                      border: `1.5px solid ${notif[key] ? sportColor : 'var(--sl-border)'}`,
                      backgroundColor: notif[key] ? `${sportColor}12` : 'var(--sl-surface)',
                      transition: 'all 0.15s',
                    }}
                  >
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button
                ...
                style={{
                  ...
                  border: '1px solid var(--sl-border)', fontSize: 14, fontWeight: 600,
              ...
              <button
                ...
                style={{
                  ...
                  boxShadow: `0 4px 16px ${sportColor}40`,
                }}
              >
                {isEditing ? 'Mettre à jour' : 'Suivre le club'}
              </button>
            </div>
          </div>
        </motion.div>
