      ...
      <AnimatePresence mode="wait">
        <motion.div key={index} initial={{ opacity: 0, scale: 0.65, y: 70 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.82, y: -35 }} transition={{ type: 'spring', stiffness: 270, damping: 22 }} style={{ textAlign: 'center', maxWidth: 310, width: '100%' }}>
          ...
          {badges.length > 1 && index < badges.length - 1 && (
            <motion.button initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.55 }} onClick={onDone} style={{ display: 'block', margin: '8px auto 0', background: 'none', border: 'none', color: 'rgba(255,255,255,0.28)', fontSize: 12, cursor: 'pointer', fontFamily: 'var(--sl-font-ui)', padding: '10px 16px' }}>Passer tout</motion.button>
          )}
        </motion.div>
      </AnimatePresence>
    </motion.div>
