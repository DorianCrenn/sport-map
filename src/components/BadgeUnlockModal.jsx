...
// 8 sparkle positions around the badge icon (radius ~68px)
const SPARKLES = [
...
function SparkleParticles({ color }) {
  return SPARKLES.map((pos, i) => (
    <motion.div
      key={i}
      initial={{ x: 0, y: 0, scale: 0, opacity: 0 }}
      animate={{ x: pos.x, y: pos.y, scale: [0, 1.3, 0], opacity: [0, 1, 0] }}
      transition={{ duration: 0.85, delay: 0.05 + i * 0.055, ease: 'easeOut' }}
      style={{
        position: 'absolute',
        width: 7, height: 7,
        borderRadius: 2,
    ...
    >
      <AnimatePresence mode="wait">
        <motion.div
          ...
          style={{ textAlign: 'center', maxWidth: 310, width: '100%' }}
        >
          {/* Progress pills */}
          {badges.length > 1 && (
            <div style={{ display: 'flex', gap: 6, justifyContent: 'center', marginBottom: 30 }}>
              {badges.map((_, i) => (
                <motion.div
                  key={i}
                  animate={{ backgroundColor: i <= index ? def.color : 'rgba(255,255,255,0.18)' }}
          ...
          <div style={{
            ...
            {/* Sparkle burst — fires on mount of each badge card */}
          ...
          {/* Skip all (only shown for multi-badge) */}
