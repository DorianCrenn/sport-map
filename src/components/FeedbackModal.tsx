...
export default function FeedbackModal({ onClose, prefilled = {} }: FeedbackModalProps) {
  ...
  const inputStyle: React.CSSProperties = { width: '100%', boxSizing: 'border-box', padding: '10px 12px', borderRadius: 'var(--sl-radius-lg)', backgroundColor: 'var(--sl-surface)', border: '1px solid var(--sl-border)', color: 'var(--sl-t1)', fontSize: 14, outline: 'none', fontFamily: 'inherit' };

  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={{ position: 'fixed', inset: 0, zIndex: (Z as any).feedbackModal, backgroundColor: 'rgba(0,0,0,0.55)', display: 'flex', alignItems: 'flex-end', fontFamily: 'var(--sl-font-ui)' }} onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
      <motion.div ref={panelRef} role="dialog" aria-modal="true" aria-label="Votre retour" initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} transition={{ type: 'spring', stiffness: 380, damping: 38 }} style={{ width: '100%', maxHeight: '92dvh', borderRadius: '20px 20px 0 0', backgroundColor: 'var(--sl-bg)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }} onClick={e => e.stopPropagation()}>
        <div style={{ flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 16px 12px', borderBottom: '1px solid var(--sl-border)' }}>
          ...
          <button onClick={onClose} aria-label="Fermer" style={{ width: 36, height: 36, borderRadius: 'var(--sl-radius-lg)', border: 'none', cursor: 'pointer', backgroundColor: 'var(--sl-surface)', color: 'var(--sl-t2)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
          ...
          <AnimatePresence mode="wait">
            {done ? (
              <motion.div key="done" initial={{ opacity: 0, scale: 0.92 }} animate={{ opacity: 1, scale: 1 }} style={{ textAlign: 'center', padding: '40px 16px' }}>
                ...
                <button onClick={onClose} style={{ padding: '12px 28px', borderRadius: 'var(--sl-radius-xl)', border: 'none', cursor: 'pointer', backgroundColor: currentType?.color ?? '#6366f1', color: '#fff', fontSize: 14, fontWeight: 700 }}>Fermer</button>
              </motion.div>
            ) : (
              <motion.form key="form" onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
                <div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
                    {TYPES.map(t => { const sel = type === t.value; return (
                      <button key={t.value} type="button" onClick={() => handleTypeChange(t.value)} style={{ padding: '10px 8px', borderRadius: 'var(--sl-radius-xl)', cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, border: `2px solid ${sel ? t.color : 'var(--sl-border)'}`, backgroundColor: sel ? t.bg : 'var(--sl-surface)', transition: 'all 0.12s' }}>
                  ...
                  <AnimatePresence>
                    {type === 'idea' && similar.length > 0 && (
                      <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} style={{ overflow: 'hidden' }}>
                        <div style={{ marginTop: 8, padding: '10px 12px', borderRadius: 'var(--sl-radius-lg)', backgroundColor: 'rgba(245,158,11,0.08)', border: '1px solid rgba(245,158,11,0.25)' }}>
                ...
                <button type="submit" disabled={!title.trim() || submitting} style={{ padding: '13px', borderRadius: 'var(--sl-radius-xl)', border: 'none', cursor: !title.trim() || submitting ? 'not-allowed' : 'pointer', backgroundColor: !title.trim() || submitting ? 'var(--sl-surface)' : (currentType?.color ?? '#6366f1'), color: !title.trim() || submitting ? 'var(--sl-t3)' : '#fff', fontSize: 14, fontWeight: 700, transition: 'all 0.15s', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
