...
function StepHeader({ step, total, title, subtitle, optional }: { step: number; total: number; title: string; subtitle?: string; optional?: boolean }) {
  return (
...
function ProgressBar({ step, total, accentColor }: { step: number; total: number; accentColor: string }) {
  return (
...
function LogoUpload({ logo, name, accentColor, onUpload, uploading }: { logo: string; name: string; accentColor: string; onUpload: (e: React.ChangeEvent<HTMLInputElement>) => void; uploading: boolean }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
      <div onClick={() => fileRef.current?.click()} style={{ width: 72, height: 72, borderRadius: 16, flexShrink: 0, backgroundColor: logo ? '#fff' : accentColor, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', overflow: 'hidden', border: `2px dashed ${logo ? 'transparent' : 'rgba(255,255,255,0.4)'}`, boxShadow: '0 2px 12px rgba(0,0,0,0.1)', position: 'relative' }}>
        ...
        <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: 0, transition: 'opacity 0.15s' }} onMouseEnter={e => { e.currentTarget.style.opacity = '1'; }} onMouseLeave={e => { e.currentTarget.style.opacity = '0'; }}>
...
function ClubPreview({ form, accentColor }: { form: Record<string, any>; accentColor: string }) {
    ...
    <div style={{ borderRadius: 14, overflow: 'hidden', border: '1px solid var(--sl-border)', backgroundColor: 'var(--sl-card)', marginBottom: 16 }}>
      <div style={{ height: 56, background: `linear-gradient(135deg, #0f172a 0%, ${accentColor}40 100%)`, display: 'flex', alignItems: 'flex-end', padding: '0 12px 8px', position: 'relative' }}>
        ...
        <div style={{ marginLeft: 8 }}>
          ...
          <div style={{ fontSize: 9, color: 'rgba(255,255,255,0.6)', lineHeight: 1.2 }}>{[form.sport, form.city].filter(Boolean).join(' · ') || 'Sport · Ville'}</div>
        </div>
      </div>
      {teamCount > 0 && <div style={{ padding: '6px 12px', fontSize: 10, color: 'var(--sl-t3)' }}>{teamCount} équipe{teamCount > 1 ? 's' : ''}</div>}
    </div>
  );
}

function RecapRow({ label, value }: { label: string; value?: string | number | null }) {
...
export default function ClubCreationWizard({ onSave, onClose }: ClubCreationWizardProps) {
  ...
  const sportList = Object.entries(SPORTS as Record<string, any>)
    .filter(([, s]: any) => !s.isArchived)
    .map(([id, s]: any) => ({ id, ...s }));

  const accentColor: string = form.primaryColor || SPORTS[form.sport]?.color || '#22C55E';
  ...
  function toggleQuickTeam(qt: { cat: string; name: string }) {
    const cats = [...(form.categories ?? [])];
    const existing = cats.find((c: any) => c.name === qt.cat);
    if (existing) {
      const hasTeam = existing.teams?.some((t: any) => t.name === qt.name);
      if (hasTeam) {
        existing.teams = (existing.teams ?? []).filter((t: any) => t.name !== qt.name);
  ...
  function isTeamSelected(qt: { cat: string; name: string }) {
    return (form.categories ?? []).some((c: any) => c.name === qt.cat && c.teams?.some((t: any) => t.name === qt.name));
  }
      ...
      <>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div>
            ...
            <input value={form.name} onChange={e => set('name', e.target.value)} placeholder="FC Brest, US Landerneau…" style={{ ...inputStyle, borderColor: errors.name ? '#ef4444' : 'var(--sl-border)' }} autoFocus />
          ...
          <div>
            ...
            <CityAutocomplete value={form.city} onChange={(val: string) => { set('city', val); setCityValid(false); }} onSelect={(commune: any) => { set('city', commune.nom); if (commune.codesPostaux?.[0]) set('postalCode', commune.codesPostaux[0]); if (commune.codeDepartement) set('region', commune.codeRegion ?? ''); setCityValid(true); }} placeholder="Paris, Lyon, Nantes…" inputStyle={{ ...inputStyle, borderColor: errors.city ? '#ef4444' : 'var(--sl-border)' }} />
            {errors.city && <div style={{ fontSize: 11, color: '#ef4444', marginTop: 4 }}>{errors.city}</div>}
          </div>
          <div>
            <label style={labelStyle}>Année de création</label>
            <input type="number" value={form.foundingYear} onChange={e => set('foundingYear', e.target.value)} placeholder="Ex : 1952" min={1800} max={new Date().getFullYear()} style={inputStyle} />
          </div>
          <div>
          ...
          <LogoUpload logo={form.logoUrl} name={form.name} accentColor={accentColor} onUpload={handleLogoUpload} uploading={logoUploading} />
        </div>
      </>
    ),
    2: (
      ...
      <motion.div initial={{ y: 60, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 60, opacity: 0 }} transition={{ type: 'spring', stiffness: 360, damping: 34 }} ref={panelRef} role="dialog" aria-modal="true" aria-label="Créer mon club" style={{ width: '100%', maxWidth: 540, backgroundColor: 'var(--sl-card)', borderRadius: '22px 22px 0 0', border: '1px solid var(--sl-border)', borderBottom: 'none', maxHeight: '92dvh', display: 'flex', flexDirection: 'column', paddingBottom: 'env(safe-area-inset-bottom, 0px)' }} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '16px 18px 8px', overscrollBehavior: 'contain' }}>
          <AnimatePresence mode="wait" initial={false}>
            <motion.div key={step} initial={{ x: dir > 0 ? 40 : -40, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: dir > 0 ? -40 : 40, opacity: 0 }} transition={{ duration: 0.18 }}>
              {stepContent[step]}
            </motion.div>
          </AnimatePresence>
        </div>
