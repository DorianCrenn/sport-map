...
const MSG_COLORS = { info: '#3b82f6', urgent: '#ef4444', cancelled: '#ef4444', rescheduled: '#f97316' };
const MSG_EMOJIS = { info: 'ℹ️', urgent: '🚨', cancelled: '❌', rescheduled: '🕐' };

export default function TrainingSessionDetail({ session, clubId, currentUser, isManager, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40" onClick={onClose}>
      <div
        className="bg-white rounded-t-3xl w-full max-w-lg overflow-y-auto"
      ...
      >
        ...
        <div className="px-6 pt-6 pb-4 border-b border-gray-100">
          ...
          <div className="flex gap-3 mt-4">
            {Object.entries(STATUS_CONFIG).map(([s, cfg]) => (
              <div key={s} className="flex-1 rounded-xl py-2 text-center"
                style={{ backgroundColor: cfg.bg }}>
                <p className="text-lg font-bold font-poppins" style={{ color: cfg.color }}>
                  {counts[s] ?? 0}
                </p>
            ...
            <div className="flex gap-2">
              {Object.entries(STATUS_CONFIG).map(([s, cfg]) => (
                <button key={s} onClick={() => respond(s)}
                  ...
                  <span className="text-[10px] font-semibold" style={{ color: myStatus === s ? cfg.color : '#9ca3af' }}>
                    {cfg.label}
                  </span>
                </button>
              ))}
        ...
        {/* Messages de la séance */}
        {messages.length > 0 && (
          <div className="px-6 py-4 border-b border-gray-100">
            <p className="text-xs font-bold uppercase tracking-wide text-gray-400 mb-3">Messages</p>
            <div className="space-y-2">
              {messages.map(m => (
                <div key={m.id} className="flex gap-3 p-3 rounded-xl"
                  style={{ backgroundColor: `${MSG_COLORS[m.type]}10`, borderLeft: `3px solid ${MSG_COLORS[m.type]}` }}>
              ...
              {attendance.filter(a => a.status === 'present').map(a => (
                <div key={a.id} className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-green-50">
                  {a.profiles?.avatar_url
                    ? <img src={a.profiles.avatar_url} className="w-5 h-5 rounded-full object-cover" alt="" />
                    : <div className="w-5 h-5 rounded-full bg-green-200 flex items-center justify-center text-[9px] font-bold text-green-700">
                        {a.profiles?.name?.slice(0, 1) ?? '?'}
                      </div>
                  }
        ...
        <div className="px-6 py-5 space-y-2">
          ...
          <button onClick={onClose}
            className="w-full py-3 rounded-2xl text-sm font-semibold border border-gray-200 text-gray-500">
            Fermer
          </button>
        </div>
      </div>
