...
interface SendTrainingMessageModalProps {
  session?: Record<string, any> | null;
  clubId?: string | number;
  userId?: string;
  onSend: (payload: Record<string, any>) => Promise<boolean | void>;
  onClose: () => void;
...
export default function SendTrainingMessageModal({
  ...
  const { hasDraft, saveDraft, loadDraft, clearDraft } = useFormDraft('sl-training-msg-draft') as any;
    ...
    <ModalFrame open variant="sheet" onClose={onClose} labelledBy="training-msg-title">
      <div ref={panelRef} style={{ display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ padding: '4px 0 14px' }}>
          <h3 id="training-msg-title" style={{ fontSize: 15, fontWeight: 800, color: 'var(--sl-t1)', margin: '0 0 3px' }}>📬 Message aux participants</h3>
          {session && (
        ...
        <div style={{ marginBottom: 16 }}>
          <label style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', color: 'var(--sl-t3)', display: 'block', marginBottom: 6 }}>Message</label>
          <textarea value={content} onChange={e => setContent(e.target.value)} rows={4} maxLength={500} placeholder="Rédigez votre message…" style={inputStyle} />
          {content.length > 400 && (
            <span style={{ fontSize: 11, color: content.length >= 500 ? '#ef4444' : 'var(--sl-t3)', display: 'block', textAlign: 'right', marginTop: 3 }}>{content.length}/500</span>
          )}
        </div>
