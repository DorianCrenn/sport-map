...
interface QuickAddTeamModalProps {
  ...
  onSave: (patch: Record<string, any>, newTeamId?: string) => Promise<void>;
  onClose: () => void;
}

export default function QuickAddTeamModal({ club, onSave, onClose }: QuickAddTeamModalProps) {
  const existingCatNames: string[] = (club.categories ?? []).map((c: any) => c.name);
  ...
  function selectCat(name: string) {
    setCatName(name);
    const existing = (club.categories ?? []).find((c: any) => c.name === name);
  ...
  async function handleSave() {
    ...
    setSaving(true);
    const cats: any[] = club.categories ?? [];
  ...
  return (
    <motion.div
      ...
      style={{ position: 'absolute', inset: 0, zIndex: 70, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', backgroundColor: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(2px)' }}
      onClick={e => e.target === e.currentTarget && onClose()}
    >
      <motion.div
        initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }}
        transition={{ type: 'spring', stiffness: 380, damping: 38 }}
        style={{ backgroundColor: 'var(--sl-card)', borderRadius: '20px 20px 0 0', padding: '0 0 env(safe-area-inset-bottom, 16px)', maxHeight: '80dvh', display: 'flex', flexDirection: 'column' }}
      >
