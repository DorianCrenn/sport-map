...
const ROLES = [
  ...
  { id: 'editor',       label: 'Éditeur',        desc: 'Events + affiches uniquement',                     color: '#f59e0b' },
  { id: 'communicant',  label: 'Communicant',    desc: 'Annonces clubs uniquement',                         color: '#22d96a' },
  { id: 'coach',        label: 'Entraîneur',     desc: 'Entraînements + équipes (lecture/gestion planning)',color: '#0ea5e9' },
];
...
interface Manager { email: string; name?: string | null; role?: string; status?: string }

interface ClubManagersPanelProps {
  managers: Manager[];
...
export default function ClubManagersPanel({ managers, ownerEmail, ownerName, clubId, clubName, onAdd, onRemove, onRoleChange, onClose }: ClubManagersPanelProps) {
  ...
  async function sendInviteEmail(normalizedEmail: string) {
    try {
      await supabase.functions.invoke('invite-club-manager', { body: { email: normalizedEmail, clubId, clubName, role, inviterName: ownerName } });
    } catch (e) {
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
          <div>
            ...
            <div style={{ fontSize: 11, color: 'var(--sl-t3)', marginTop: 2 }}>Gérez les accès à votre club</div>
          </div>
          <button onClick={onClose} aria-label="Fermer" style={{ width: 44, height: 44, borderRadius: '50%', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: 'var(--sl-surface)', color: 'var(--sl-t3)' }}>
        ...
        {managers.map(m => {
          const rm = roleMeta(m.role ?? 'manager');
        ...
        {managers.length === 0 && (
          <p style={{ fontSize: 12, color: 'var(--sl-t3)', textAlign: 'center', padding: '12px 0', marginBottom: 4 }}>Aucun gestionnaire délégué pour l'instant</p>
        )}

        <div style={{ height: 1, backgroundColor: 'var(--sl-border)', margin: '12px 0' }} />

        <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--sl-t3)', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 8 }}>Inviter un gestionnaire</div>
        ...
        <p style={{ fontSize: 11, color: 'var(--sl-t3)', marginTop: 8 }}>La personne doit avoir un compte SportLink avec cet email pour accéder à l'édition.</p>
      </motion.div>
    </div>
  );
}
