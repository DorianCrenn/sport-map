...
const BLANK: Match = { id: '', date: '', time: '15:00', opponent: '', isHome: true, competition: '', teamId: '', teamName: '', venue: '', scoreHome: '', scoreAway: '', publishedOnMap: true, supabaseEventId: null };

function matchToEvent(m: Match, club: Record<string, any>) {
...
function getResult(match: Match): string | null {
...
const RESULT_COLORS: Record<string, string> = { V: '#22C55E', D: '#ef4444', N: '#f59e0b' };

function MatchCard({ match, club, isEditing, onEdit, onRemove }: { match: Match; club?: Record<string, any> | null; isEditing?: boolean; onEdit: () => void; onRemove: () => void }) {
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5 mb-0.5 flex-wrap">
            <span className="text-[9px] font-bold px-1.5 py-0.5 rounded"
              style={match.isHome ? { backgroundColor: '#EFF6FF', color: '#1d4ed8' } : { backgroundColor: '#F8FAFC', color: '#64748b' }}>
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          {hasScore ? (
            <div className="flex items-center gap-1.5">
              ...
              {result && (
                <span className="text-[10px] font-bold px-1.5 py-0.5 rounded text-white" style={{ backgroundColor: RESULT_COLORS[result] }}>{result}</span>
              )}
            </div>
          ) : upcoming ? (
          ...
          {isEditing && !match._fromMap && (
            <div className="flex items-center gap-0.5 ml-1">
              <button onClick={onEdit} className="p-1.5 text-gray-300 hover:text-blue-500 transition-colors rounded-lg hover:bg-blue-50">
              ...
              <button onClick={onRemove} className="p-1.5 text-gray-300 hover:text-red-500 transition-colors rounded-lg hover:bg-red-50">
...
function MatchForm({ initial, allTeams, onSave, onCancel }: { initial: Match; allTeams: Team[]; onSave: (f: Match) => void; onCancel: () => void }) {
  const [f, setF] = useState<Match>(initial);
  function set(k: keyof Match, v: any) { setF(p => ({ ...p, [k]: v })); }
    ...
    >
      ...
      <div>
        ...
        <div className="flex items-center gap-2">
          <input type="number" min="0" value={f.scoreHome ?? ''} onChange={e => set('scoreHome', e.target.value)}
            placeholder="–" className="w-14 text-center text-sm font-bold border border-gray-200 rounded-xl px-2 py-2 bg-white focus:outline-none" />
      ...
      {f.isHome && (
        <button
          ...
          style={f.publishedOnMap ? { backgroundColor: '#f0fdf4', borderColor: '#86efac' } : { backgroundColor: '#f8fafc', borderColor: '#e2e8f0' }}
        >
          <div className="flex items-center gap-2">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ color: f.publishedOnMap ? '#16a34a' : '#94a3b8' }}>
              <polygon points="3 6 9 3 15 6 21 3 21 18 15 21 9 18 3 21"/>
              <line x1="9" y1="3" x2="9" y2="18"/><line x1="15" y1="6" x2="15" y2="21"/>
            </svg>
            <span className="text-xs font-semibold" style={{ color: f.publishedOnMap ? '#16a34a' : '#64748b' }}>Publier sur la carte</span>
          </div>
          <div className="w-9 h-5 rounded-full transition-colors flex items-center px-0.5 flex-shrink-0" style={{ backgroundColor: f.publishedOnMap ? '#22c55e' : '#d1d5db' }}>
            <div className="w-4 h-4 bg-white rounded-full shadow transition-transform" style={{ transform: f.publishedOnMap ? 'translateX(16px)' : 'translateX(0)' }} />
          </div>
        </button>
      )}

      <div className="flex gap-2 pt-1">
        <button onClick={onCancel} className="flex-1 py-2.5 rounded-xl border border-gray-200 text-sm font-semibold text-gray-500 hover:bg-gray-100 bg-white transition-colors">
          Annuler
        </button>
...
function eventToMatch(event: Record<string, any>, club: Record<string, any>): Match {
...
export default function MatchesBlock({ data, onUpdate, isEditing, club, filterTeamId, allEvents }: MatchesBlockProps) {
  ...
  const matches = filterTeamId
    ? merged.filter(m => m.teamId === filterTeamId || (m._fromMap && m.teamName === filterTeam?.name))
    : merged;

  const [showForm, setShowForm] = useState(false);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [formInit, setFormInit] = useState<Match>(BLANK);
  ...
  async function handleSave(f: Match) {
    let cleaned: Match = { ...f, scoreHome: f.scoreHome === '' ? null : Number(f.scoreHome), scoreAway: f.scoreAway === '' ? null : Number(f.scoreAway) };
    ...
    if (club) {
      if (cleaned.publishedOnMap && cleaned.isHome && cleaned.date) {
        ...
        if (cleaned.supabaseEventId) {
          ...
          const { data: inserted } = await supabase.from('events').insert(eventData).select('id').single();
          if (inserted?.id) cleaned = { ...cleaned, supabaseEventId: inserted.id };
        }
      ...
      {upcoming.length > 0 && (
        <div>
          ...
          <div className="space-y-2">
            {upcoming.map(m => (
              <MatchCard key={m.id} match={m} club={club} isEditing={isEditing} onEdit={() => openEdit(m)} onRemove={() => remove(m.id)} />
            ))}
          </div>
        </div>
      )}
