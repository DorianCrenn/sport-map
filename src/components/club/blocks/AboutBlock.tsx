...
const CONTACT_META: Record<string, { color: string; bg: string }> = {
...
function ContactIcon({ type, size = 16 }: { type: string; size?: number }) {
  ...
  switch (type) {
    ...
    case 'youtube':
      return <svg width={s} height={s} viewBox="0 0 24 24" fill="currentColor"><path d="M22.54 6.42a2.78 2.78 0 0 0-1.95-1.96C18.88 4 12 4 12 4s-6.88 0-8.59.46A2.78 2.78 0 0 0 1.46 6.42 29 29 0 0 0 1 12a29 29 0 0 0 .46 5.58A2.78 2.78 0 0 0 3.41 19.6C5.12 20 12 20 12 20s6.88 0 8.59-.46a2.78 2.78 0 0 0 1.95-1.95A29 29 0 0 0 23 12a29 29 0 0 0-.46-5.58z"/><polygon points="9.75 15.02 15.5 12 9.75 8.98 9.75 15.02" fill="white"/></svg>;
    case 'tiktok':
...
interface Price { id: number; label: string; amount: string }
interface Contact { id: number; type: string; value: string }
...
function PriceRow({ price, onChange, onDelete }: { price: Price; onChange: (p: Price) => void; onDelete: () => void }) {
  return (
...
function ContactRow({ contact, onChange, onDelete }: { contact: Contact; onChange: (c: Contact) => void; onDelete: () => void }) {
...
export function AboutBlockEditor({ block, onChange }: { block: Record<string, any>; onChange: (b: Record<string, any>) => void }) {
  const data = block.data || {};
  function set(key: string, val: any) { onChange({ ...block, data: { ...data, [key]: val } }); }
  function addPrice() { set('prices', [...(data.prices || []), { id: Date.now(), label: '', amount: '' }]); }
  function updatePrice(id: number, u: Price) { set('prices', (data.prices || []).map((p: Price) => p.id === id ? u : p)); }
  function deletePrice(id: number) { set('prices', (data.prices || []).filter((p: Price) => p.id !== id)); }
  function addContact() { set('contacts', [...(data.contacts || []), { id: Date.now(), type: 'email', value: '' }]); }
  function updateContact(id: number, u: Contact) { set('contacts', (data.contacts || []).map((c: Contact) => c.id === id ? u : c)); }
  function deleteContact(id: number) { set('contacts', (data.contacts || []).filter((c: Contact) => c.id !== id)); }
...
export function AboutBlockView({ block }: { block: Record<string, any> }) {
  const data = block.data || {};
  ...
  const socialContacts  = (data.contacts || []).filter((c: Contact) => SOCIAL_TYPES.has(c.type) && c.value);
  const classicContacts = (data.contacts || []).filter((c: Contact) => !SOCIAL_TYPES.has(c.type) && c.value);
        ...
        <div>
          ...
          {socialContacts.length > 0 && (
            <div className="flex flex-wrap gap-2.5 mb-3">
              {(socialContacts as Contact[]).map(c => {
          ...
          {classicContacts.length > 0 && (
            <div className="flex flex-col gap-2">
              {(classicContacts as Contact[]).map(c => {
