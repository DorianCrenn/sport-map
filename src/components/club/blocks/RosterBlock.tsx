...
function PlayerCard({ player, currentUser, onClaim }: { player: Player; currentUser?: Record<string, any> | null; onClaim?: (p: Player) => void }) {
  ...
  return (
    <div className="flex items-center gap-3 p-3 rounded-2xl border border-gray-100 bg-white hover:border-gray-200 transition-colors">
      ...
      {player.photo_url
        ? <img src={player.photo_url} alt={player.name} loading="lazy" className="w-9 h-9 rounded-full object-cover flex-shrink-0" />
        : (
...
export default function RosterBlock({ data, clubId, currentUser }: RosterBlockProps) {
  const teamId = data?.teamId ?? null;
  const { players, loading } = useClubPlayers(String(clubId), teamId) as { players: Player[]; loading: boolean };
  ...
  const byPosition = players.reduce((acc, p) => {
    const key = ORDER.includes(p.position ?? '') ? (p.position as string) : 'Autre';
    if (!acc[key]) acc[key] = [];
    acc[key].push(p);
    return acc;
