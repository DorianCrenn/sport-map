...
export default function SendAnnouncementModal({ club, onSend, onClose }: SendAnnouncementModalProps) {
  ...
  const { hasDraft, saveDraft, loadDraft, clearDraft } = useFormDraft('sl-announcement-draft') as any;
  ...
  async function handleAISuggest() {
    ...
    try {
      const { data, error: fnErr } = await import('../../lib/supabase.js').then(m =>
        m.supabase.functions.invoke('generate-announcement', { body: { clubName: club.name, type } })
      );
  ...
  const allTeams = (club.categories ?? []).flatMap((cat: any) =>
    (cat.teams ?? []).map((team: any) => ({ id: team.id ?? team.name, name: team.name, catName: cat.name }))
  );
  ...
  async function handleSend() {
    const { ok, errors } = validate(announcementSchema, { type, message }) as any;
    if (!ok) {
    ...
    >
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 18px 14px', borderBottom: '1px solid var(--sl-border)', flexShrink: 0 }}>
          <div>
            <h2 style={{ fontSize: 16, fontWeight: 800, color: 'var(--sl-t1)', margin: 0, letterSpacing: '-0.02em' }}>📢 Envoyer une annonce</h2>
            <div style={{ fontSize: 11, color: 'var(--sl-t3)', marginTop: 3 }}>{club.name}</div>
          </div>
          <button onClick={onClose} style={{ width: 44, height: 44, borderRadius: 'var(--sl-radius-xl)', border: 'none', cursor: 'pointer', backgroundColor: 'var(--sl-surface)', color: 'var(--sl-t2)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '16px 18px 8px', display: 'flex', flexDirection: 'column', gap: 16, overscrollBehavior: 'contain' }}>
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 6 }}>
              {TYPE_OPTIONS.map(opt => {
                ...
                return (
                  <button key={opt.key} onClick={() => setType(opt.key)} style={{ padding: '10px 4px', borderRadius: 'var(--sl-radius-xl)', cursor: 'pointer', textAlign: 'center', border: `2px solid ${sel ? opt.color : 'var(--sl-border)'}`, backgroundColor: sel ? opt.bg : 'var(--sl-surface)', transition: 'all 0.12s' }}>
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: 6, overflowX: 'auto', paddingBottom: 4, scrollbarWidth: 'none' }}>
              {(TEMPLATES[type] ?? []).map(tpl => (
                <button key={tpl.label} onClick={() => setMessage(tpl.text)} style={{ flexShrink: 0, padding: '6px 12px', borderRadius: 'var(--sl-radius-4xl)', cursor: 'pointer', fontSize: 11, fontWeight: 600, whiteSpace: 'nowrap', border: `1.5px solid ${selectedType?.color}40`, backgroundColor: message === tpl.text ? selectedType?.bg : 'var(--sl-surface)', color: message === tpl.text ? selectedType?.color : 'var(--sl-t2)', transition: 'all 0.12s' }}>
                  {tpl.label}
                </button>
              ))}
            </div>
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
              ...
              <button onClick={handleAISuggest} disabled={aiLoading} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '5px 12px', borderRadius: 'var(--sl-radius-4xl)', border: `1.5px solid ${selectedType?.color}40`, backgroundColor: aiLoading ? 'var(--sl-surface)' : `${selectedType?.color ?? '#a855f7'}12`, color: aiLoading ? 'var(--sl-t3)' : (selectedType?.color ?? '#a855f7'), fontSize: 11, fontWeight: 700, cursor: aiLoading ? 'default' : 'pointer', transition: 'all 0.15s' }}>
                {aiLoading ? <><span style={{ display: 'inline-block', width: 10, height: 10, borderRadius: '50%', border: '2px solid currentColor', borderTopColor: 'transparent', animation: 'sl-spin 0.6s linear infinite' }}/>Génération…</> : '✨ Générer'}
              </button>
            </div>
            {aiSuggestions.length > 0 && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {aiSuggestions.map((s, i) => (
                  <button key={i} onClick={() => { setMessage(s); setAiSuggestions([]); }} style={{ textAlign: 'left', padding: '10px 12px', borderRadius: 'var(--sl-radius-lg)', border: `1.5px solid ${selectedType?.color ?? '#a855f7'}30`, backgroundColor: 'var(--sl-surface)', color: 'var(--sl-t1)', fontSize: 12, cursor: 'pointer', lineHeight: 1.5, fontFamily: 'var(--sl-font-ui)', transition: 'all 0.12s' }}>
                    {s}
                  </button>
                ))}
              </div>
          ...
          <div>
            ...
            <textarea value={message} onChange={e => setMessage(e.target.value)} placeholder="Tapez votre message…" rows={3} style={inputStyle} autoFocus />
          ...
          {allTeams.length > 0 && (
            <div>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                <button onClick={() => setTargets([])} style={{ padding: '6px 14px', borderRadius: 'var(--sl-radius-4xl)', cursor: 'pointer', fontSize: 12, fontWeight: isAll ? 700 : 500, border: `2px solid ${isAll ? 'var(--sl-green)' : 'var(--sl-border)'}`, backgroundColor: isAll ? 'rgba(34,217,106,0.12)' : 'var(--sl-surface)', color: isAll ? 'var(--sl-green)' : 'var(--sl-t2)', transition: 'all 0.12s' }}>
                  ✓ Tous les abonnés
                </button>
                {allTeams.map((team: any) => {
                  const sel = targets.includes(team.id);
                  return (
                    <button key={team.id} onClick={() => { if (isAll) setTargets([team.id]); else toggleTarget(team.id); }} style={{ padding: '6px 14px', borderRadius: 'var(--sl-radius-4xl)', cursor: 'pointer', fontSize: 12, fontWeight: sel ? 700 : 500, border: `2px solid ${sel ? '#3b82f6' : 'var(--sl-border)'}`, backgroundColor: sel ? 'rgba(59,130,246,0.1)' : 'var(--sl-surface)', color: sel ? '#3b82f6' : 'var(--sl-t2)', transition: 'all 0.12s' }}>
                      {team.name}
                    </button>
                  );
                })}
              </div>
              {!isAll && <div style={{ fontSize: 11, color: 'var(--sl-t3)', marginTop: 6 }}>{targets.length} équipe{targets.length > 1 ? 's' : ''} sélectionnée{targets.length > 1 ? 's' : ''}</div>}
            </div>
          )}

          <div style={{ borderRadius: 'var(--sl-radius-2xl)', border: '1px solid var(--sl-border)', overflow: 'hidden' }}>
            <button onClick={() => setScheduled(v => !v)} style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '11px 14px', border: 'none', cursor: 'pointer', backgroundColor: scheduled ? 'rgba(168,85,247,0.08)' : 'var(--sl-surface)', transition: 'background 0.15s' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{ fontSize: 15 }}>🕐</span>
                <div style={{ textAlign: 'left' }}>
                  <div style={{ fontSize: 13, fontWeight: 700, color: scheduled ? '#a855f7' : 'var(--sl-t1)' }}>Programmer l'envoi</div>
                  {scheduled && scheduleAt && <div style={{ fontSize: 10, color: '#a855f7', marginTop: 1 }}>{new Date(scheduleAt).toLocaleString('fr-FR', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' })}</div>}
                </div>
              </div>
              <div style={{ width: 28, height: 16, borderRadius: 'var(--sl-radius-md)', background: scheduled ? '#a855f7' : 'rgba(255,255,255,0.1)', position: 'relative', transition: 'background 0.15s', flexShrink: 0 }}>
            ...
            {scheduled && (
              <div style={{ padding: '0 14px 12px', borderTop: '1px solid var(--sl-border)' }}>
                ...
                <input type="datetime-local" value={scheduleAt} min={scheduleAtMin} onChange={e => setScheduleAt(e.target.value)} style={{ width: '100%', boxSizing: 'border-box', padding: '9px 11px', borderRadius: 'var(--sl-radius-lg)', fontSize: 13, border: '1px solid var(--sl-border)', backgroundColor: 'var(--sl-surface)', color: 'var(--sl-t1)', outline: 'none' }} />
              </div>
            )}
          </div>
        ...
        <div style={{ flexShrink: 0, padding: '14px 18px 20px', borderTop: '1px solid var(--sl-border)', display: 'flex', gap: 10 }}>
          <button onClick={onClose} style={{ flex: 1, padding: '13px 0', borderRadius: 'var(--sl-radius-2xl)', border: '1px solid var(--sl-border-s)', color: 'var(--sl-t2)', backgroundColor: 'var(--sl-surface)', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>Annuler</button>
          <button onClick={handleSend} disabled={saving || !message.trim()} style={{ flex: 2, padding: '13px 0', borderRadius: 'var(--sl-radius-2xl)', border: 'none', fontSize: 14, fontWeight: 700, cursor: saving || !message.trim() ? 'default' : 'pointer', backgroundColor: saving || !message.trim() ? 'var(--sl-surface)' : (selectedType?.color ?? 'var(--sl-green)'), color: saving || !message.trim() ? 'var(--sl-t3)' : '#fff', boxShadow: saving || !message.trim() ? 'none' : `0 4px 14px ${selectedType?.color ?? '#22d96a'}55`, transition: 'all 0.15s' }}>
