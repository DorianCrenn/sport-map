    ...
    >
      ...
      >
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '16px 18px 8px', display: 'flex', flexDirection: 'column', gap: 16, overscrollBehavior: 'contain' }}>
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
              ...
              <button onClick={handleAISuggest} disabled={aiLoading} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '5px 12px', borderRadius: 20, border: `1.5px solid ${selectedType?.color}40`, backgroundColor: aiLoading ? 'var(--sl-surface)' : `${selectedType?.color ?? '#a855f7'}12`, color: aiLoading ? 'var(--sl-t3)' : (selectedType?.color ?? '#a855f7'), fontSize: 11, fontWeight: 700, cursor: aiLoading ? 'default' : 'pointer', transition: 'all 0.15s' }}>
          ...
          {allTeams.length > 0 && (
            <div>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                <button onClick={() => setTargets([])} style={{ padding: '6px 14px', borderRadius: 20, cursor: 'pointer', fontSize: 12, fontWeight: isAll ? 700 : 500, border: `2px solid ${isAll ? 'var(--sl-green)' : 'var(--sl-border)'}`, backgroundColor: isAll ? 'rgba(34,217,106,0.12)' : 'var(--sl-surface)', color: isAll ? 'var(--sl-green)' : 'var(--sl-t2)', transition: 'all 0.12s' }}>
                  ✓ Tous les abonnés
                </button>
          ...
          <div style={{ borderRadius: 14, border: '1px solid var(--sl-border)', overflow: 'hidden' }}>
            ...
            {scheduled && (
              <div style={{ padding: '0 14px 12px', borderTop: '1px solid var(--sl-border)' }}>
                ...
                <input type="datetime-local" value={scheduleAt} min={scheduleAtMin} onChange={e => setScheduleAt(e.target.value)} style={{ width: '100%', boxSizing: 'border-box', padding: '9px 11px', borderRadius: 10, fontSize: 13, border: '1px solid var(--sl-border)', backgroundColor: 'var(--sl-surface)', color: 'var(--sl-t1)', outline: 'none' }} />
              </div>
            )}
          </div>
        ...
        <div style={{ flexShrink: 0, padding: '14px 18px 20px', borderTop: '1px solid var(--sl-border)', display: 'flex', gap: 10 }}>
          <button onClick={onClose} style={{ flex: 1, padding: '13px 0', borderRadius: 14, border: '1px solid var(--sl-border-s)', color: 'var(--sl-t2)', backgroundColor: 'var(--sl-surface)', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>Annuler</button>
          <button onClick={handleSend} disabled={saving || !message.trim()} style={{ flex: 2, padding: '13px 0', borderRadius: 14, border: 'none', fontSize: 14, fontWeight: 700, cursor: saving || !message.trim() ? 'default' : 'pointer', backgroundColor: saving || !message.trim() ? 'var(--sl-surface)' : (selectedType?.color ?? 'var(--sl-green)'), color: saving || !message.trim() ? 'var(--sl-t3)' : '#fff', boxShadow: saving || !message.trim() ? 'none' : `0 4px 14px ${selectedType?.color ?? '#22d96a'}55`, transition: 'all 0.15s' }}>
