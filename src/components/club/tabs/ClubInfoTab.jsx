export default function ClubInfoTab({ club, accentColor }) {
    ...
    <div style={{ padding: '14px 14px calc(90px + env(safe-area-inset-bottom, 0px))', display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      {club.description && (
        <div style={{ padding: '14px 16px', borderRadius: 14, backgroundColor: 'var(--sl-card)', border: '1px solid var(--sl-border)' }}>
          ...
          <p style={{ fontSize: 13, color: 'var(--sl-t1)', lineHeight: 1.6, margin: 0 }}>
            {club.description}
          </p>
        </div>
      )}
      ...
      <div style={{ padding: '14px 16px', borderRadius: 14, backgroundColor: 'var(--sl-card)', border: '1px solid var(--sl-border)' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {[
            ...
            teamCount > 0 && { icon: '👥', label: 'Équipes', value: `${teamCount} équipe${teamCount > 1 ? 's' : ''} · ${catCount} catégorie${catCount > 1 ? 's' : ''}` },
            club.members && { icon: '❤️', label: 'Abonnés', value: `${club.members} membre${club.members > 1 ? 's' : ''}` },
            club.level && { icon: '🏆', label: 'Niveau', value: club.level },
        ...
        <div style={{ padding: '14px 16px', borderRadius: 14, backgroundColor: 'var(--sl-card)', border: '1px solid var(--sl-border)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{
              ...
              backgroundColor: `${accentColor}12`, border: `1px solid ${accentColor}25`,
      ...
      {catCount > 0 && (
        <div style={{ padding: '14px 16px', borderRadius: 14, backgroundColor: 'var(--sl-card)', border: '1px solid var(--sl-border)' }}>
          <div style={{ fontSize: 10, fontWeight: 800, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--sl-t3)', marginBottom: 10 }}>
            Sections & Équipes
          </div>
