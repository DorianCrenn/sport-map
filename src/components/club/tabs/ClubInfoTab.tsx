...
export default function ClubInfoTab({ club, accentColor }: ClubInfoTabProps) {
  ...
  return (
    <div style={{ padding: '14px 14px calc(90px + env(safe-area-inset-bottom, 0px))', display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      {club.description && (
        <div style={{ padding: '14px 16px', borderRadius: 'var(--sl-radius-2xl)', backgroundColor: 'var(--sl-card)', border: '1px solid var(--sl-border)' }}>
      ...
      {club.contact && (
        <div style={{ padding: '14px 16px', borderRadius: 'var(--sl-radius-2xl)', backgroundColor: 'var(--sl-card)', border: '1px solid var(--sl-border)' }}>
          ...
          <a
            ...
            style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', borderRadius: 'var(--sl-radius-xl)', backgroundColor: `${accentColor}10`, border: `1px solid ${accentColor}25`, textDecoration: 'none', minHeight: 44 }}
          >
      ...
      {club.city && (
        <div style={{ padding: '14px 16px', borderRadius: 'var(--sl-radius-2xl)', backgroundColor: 'var(--sl-card)', border: '1px solid var(--sl-border)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{ width: 42, height: 42, borderRadius: 'var(--sl-radius-xl)', flexShrink: 0, backgroundColor: `${accentColor}12`, border: `1px solid ${accentColor}25`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              ...
              <div key={cat.id}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5 }}>
                  {(cat.teams ?? []).map((team: any) => (
                    <span key={team.id} style={{ padding: '3px 10px', borderRadius: 'var(--sl-radius-4xl)', fontSize: 10, fontWeight: 600, backgroundColor: 'var(--sl-surface)', border: '1px solid var(--sl-border)', color: 'var(--sl-t2)' }}>
