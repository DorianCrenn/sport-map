...
interface ClubRosterTabProps {
  ...
  onUpdateTrainings?: (t: Record<string, any>[]) => void;
  onAddEventForTeam?: (teamId: string) => void;
  canAddEvent?: boolean;
  onBulkAddTrainingEvents?: () => void;
  accentColor?: string;
  canEdit?: boolean;
...
export default function ClubRosterTab({
  ...
  async function handleTeamSaved(patch: Record<string, any>, newTeamId?: string) {
      ...
      <div style={{ padding: '14px 14px calc(90px + env(safe-area-inset-bottom, 0px))', position: 'relative' }}>
        <div style={{ textAlign: 'center', padding: '40px 20px' }}>
          ...
          {canEdit ? (
            ...
            <div style={{ fontSize: 12, color: 'var(--sl-t3)', lineHeight: 1.5 }}>Aucune équipe n'a encore été ajoutée.</div>
          )}
        </div>
        <AnimatePresence>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
      <div style={{ flexShrink: 0, borderBottom: '1px solid var(--sl-border)', backgroundColor: 'var(--sl-card)', position: 'relative' }}>
        ...
        <div ref={tabBarRef} style={{ display: 'flex', overflowX: 'auto', scrollbarWidth: 'none', WebkitOverflowScrolling: 'touch' } as React.CSSProperties}>
          <button
            data-active={activeTeamId === null}
            onClick={() => setActiveTeamId(null)}
            style={{ flexShrink: 0, padding: '10px 16px', fontSize: 12, fontWeight: 700, border: 'none', background: 'none', cursor: 'pointer', borderBottom: `2px solid ${activeTeamId === null ? accentColor : 'transparent'}`, color: activeTeamId === null ? 'var(--sl-t1)' : 'var(--sl-t3)', transition: 'all 0.15s', whiteSpace: 'nowrap' }}
          >
            Général
          </button>
          {allTeams.map(team => (
            <button
              ...
              style={{ flexShrink: 0, padding: '10px 16px', fontSize: 12, fontWeight: 700, border: 'none', background: 'none', cursor: 'pointer', borderBottom: `2px solid ${activeTeamId === team.id ? accentColor : 'transparent'}`, color: activeTeamId === team.id ? 'var(--sl-t1)' : 'var(--sl-t3)', transition: 'all 0.15s', whiteSpace: 'nowrap' }}
            >
              {team.name}
            </button>
          ))}
          {canEdit && (
            <button onClick={() => setShowAddTeam(true)} title="Créer une équipe" style={{ flexShrink: 0, padding: '10px 14px', fontSize: 16, fontWeight: 700, lineHeight: 1, border: 'none', background: 'none', cursor: 'pointer', color: 'var(--sl-t3)' }}>+</button>
          )}
          <div style={{ flexShrink: 0, width: 8 }} />
        </div>
        {allTeams.length > 2 && (
          <div style={{ position: 'absolute', right: 0, top: 0, bottom: 0, width: 28, pointerEvents: 'none', background: 'linear-gradient(to right, transparent, var(--sl-card))' }} />
        )}
      </div>
