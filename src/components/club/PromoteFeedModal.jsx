...
// ── Config plans affichée à l'admin ───────────────────────────────────────────
const PLANS = [
  {
    id: 'starter',
    emoji: '⭐',
    desc: '1 événement · 7 jours',
    price: 'Gratuit',
  },
  {
    id: 'pro',
    emoji: '🔥',
    desc: '3 événements · 30 jours',
    price: 'Club Pro',
  },
  {
    id: 'elite',
    emoji: '👑',
    desc: '5 événements · 60 jours',
    price: 'Élite',
  },
];
...
// ── Sous-composant : carte plan sélectionnable ────────────────────────────────
function PlanCard({ plan, selected, onSelect }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ fontSize: 10, color: 'var(--sl-t3)', marginTop: 2, lineHeight: 1.4 }}>{meta.desc}</div>
      <div style={{
        marginTop: 6, fontSize: 10, fontWeight: 700,
        padding: '2px 8px', borderRadius: 20, display: 'inline-block',
        backgroundColor: selected ? `${cfg.accent}20` : 'var(--sl-surface)',
        color: selected ? cfg.accent : 'var(--sl-t3)',
      }}>
        {meta.price}
      </div>
    </button>
...
// ── Section principale dans le dashboard ─────────────────────────────────────
export function FeaturedSection({ club, upcomingEvents = [] }) {
  ...
  const cfg = (plan) => PLAN_CONFIG[plan];
        ...
        {!showForm && futureEvents.length > 0 && (
          <button
            ...
            style={{
              ...
              backgroundColor: '#f59e0b', color: 'white', fontSize: 12, fontWeight: 700,
            }}
          >
            + Promouvoir
          </button>
          ...
          {active.map((feat, idx) => {
            ...
            return (
              <div key={feat.id} style={{
                ...
                borderBottom: idx < active.length - 1 ? '1px solid var(--sl-border)' : 'none',
              }}>
                <div style={{
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--sl-t1)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {feat.event_title ?? feat.event_id}
                  </div>
                  <div style={{ fontSize: 10, color: 'var(--sl-t3)', marginTop: 1 }}>
                    <span style={{ color: c.accent, fontWeight: 700 }}>{c.label}</span>
                    {' · '}Jusqu'au {fmtDate(feat.ends_at)}
                  </div>
                </div>
                <button
                  onClick={() => cancel(feat.id)}
                  disabled={saving}
                  style={{
                    padding: '4px 10px', borderRadius: 8, border: '1px solid var(--sl-border)',
                    backgroundColor: 'transparent', color: 'var(--sl-t3)', fontSize: 11,
      ...
      <AnimatePresence>
        {success && (
          <motion.div
            ...
            style={{ padding: '10px 16px', backgroundColor: 'rgba(34,217,106,0.08)', borderTop: '1px solid var(--sl-border)' }}
          >
            <div style={{ fontSize: 12, fontWeight: 600, color: '#22d96a' }}>
              ✓ Événement mis en avant — il apparaît dès maintenant dans le feed.
            </div>
          </motion.div>
        )}
      </AnimatePresence>
            ...
            <div style={{ padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div>
                <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--sl-t2)', marginBottom: 6 }}>
                  Quel événement mettre en avant ?
                </div>
                    ...
                    {futureEvents.slice(0, 6).map(ev => (
                      <button
                        ...
                        style={{
                          ...
                          border: `2px solid ${selectedEvent === ev.id ? '#f59e0b' : 'var(--sl-border)'}`,
                          backgroundColor: selectedEvent === ev.id ? 'rgba(245,158,11,0.08)' : 'var(--sl-surface)',
                          cursor: 'pointer',
                        }}
                      >
              ...
              <div>
                ...
                <div style={{ fontSize: 10, color: 'var(--sl-t3)', marginTop: 8 }}>
                  Durée : <strong style={{ color: cfg(selectedPlan).accent }}>{PLAN_CONFIG[selectedPlan].durationDays} jours</strong>
                  {' · '}Expire le{' '}
                  <strong>{fmtDate(computeEndsAt(selectedPlan))}</strong>
                </div>
              </div>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <button
                  onClick={() => { setShowForm(false); setSelectedEvent(''); setSelectedPlan('starter'); }}
                  style={{
                    flex: 1, padding: '10px 0', borderRadius: 10,
                    border: '1px solid var(--sl-border)', backgroundColor: 'transparent',
                    color: 'var(--sl-t2)', fontSize: 12, fontWeight: 600, cursor: 'pointer',
                  }}
                >
                  Annuler
                </button>
                <button
                  ...
                  style={{
                    ...
                    backgroundColor: selectedEvent ? cfg(selectedPlan).accent : 'var(--sl-border)',
                    color: selectedEvent ? 'white' : 'var(--sl-t3)',
                    fontSize: 12, fontWeight: 700,
                    cursor: !selectedEvent || submitting ? 'not-allowed' : 'pointer',
                    opacity: submitting ? 0.7 : 1,
                    boxShadow: selectedEvent ? `0 4px 12px ${cfg(selectedPlan).glow}` : 'none',
                    transition: 'all 0.15s',
                  }}
                >
