            ...
            {filtered.map(p => (
              <div key={p.id} className="flex items-center gap-3 px-4 py-3 rounded-xl bg-gray-50 hover:bg-gray-100 transition-colors group">
                ...
                <button
                  ...
                  className="opacity-0 group-hover:opacity-100 p-1.5 rounded-lg text-red-400 hover:text-red-600 transition-all">
                  <XIcon />
                </button>
              </div>
            ))}
            {!filtered.length && (
              <p className="text-sm text-gray-400 text-center py-3 italic">Aucun joueur pour cette équipe.</p>
            )}
          </div>
        )
      }
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="col-span-2">
              <input required value={form.name} onChange={e => setField('name', e.target.value)}
                placeholder="Prénom Nom *" className="w-full px-3 py-2.5 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-blue-400 bg-white" />
            </div>
            <input type="number" value={form.number} onChange={e => setField('number', e.target.value)}
              placeholder="N° maillot" min={1} max={99} className="px-3 py-2.5 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-blue-400 bg-white" />
            ...
            {teams.length > 0 && (
              <select value={form.team_id} onChange={e => setField('team_id', e.target.value)}
                className="col-span-2 px-3 py-2.5 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-blue-400 bg-white">
                <option value="">Équipe (optionnel)</option>
                {teams.map(t => <option key={t.id} value={t.id}>{t.name}</option>)}
              </select>
            )}
            <input value={form.email} onChange={e => setField('email', e.target.value)}
              type="email" placeholder="Email (pour invitation)" className="col-span-2 px-3 py-2.5 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-blue-400 bg-white" />
          </div>

          <div className="flex gap-2">
            <button type="button" onClick={() => { setAdding(false); setForm(EMPTY_FORM); }}
              className="flex-1 py-2.5 rounded-xl border border-gray-200 text-sm font-semibold text-gray-500 bg-white">
              Annuler
            </button>
