...
export default function MobileEventSheet({ event, onClose, onEdit, onDelete, isFavorite, onToggleFavorite, isAttending, onToggleAttend }) {
    ...
    >
      ...
      <div style={{ overflowY: 'auto', maxHeight: 'calc(80vh - 56px)' }}>
        <div style={{ padding: '0 16px 32px' }}>
          ...
          {/* Top row: sport badge + status + actions */}
          ...
          {/* Title */}
          <h2 style={{ fontWeight: 800, fontSize: 20, lineHeight: 1.2, letterSpacing: '-0.02em', color: 'var(--sl-t1)', marginBottom: 14, fontFamily: 'Inter, sans-serif' }}>
            {event.title}
          </h2>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 16 }}>
            ...
            {(typeMeta || event.teamName) && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, paddingLeft: 42 }}>
                ...
                {event.teamName && <span style={{ fontSize: 12, color: 'var(--sl-t2)', fontWeight: 500 }}>{event.teamName}</span>}
                {event.category && <span style={{ fontSize: 11, color: 'var(--sl-t3)' }}>({event.category})</span>}
              </div>
            )}
          </div>
