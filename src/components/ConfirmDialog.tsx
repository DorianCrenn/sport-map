  ...
  return (
    <ModalFrame open={open} onClose={onCancel} variant="sheet" zIndex={500} labelledBy="confirm-dlg-title">
      ...
      <div style={{ display: 'flex', gap: 10 }}>
        <button onClick={onCancel} style={{ flex: 1, padding: '12px 0', borderRadius: 'var(--sl-radius-xl)', cursor: 'pointer', fontSize: 14, fontWeight: 600, border: '1px solid var(--sl-border-s)', color: 'var(--sl-t2)', backgroundColor: 'var(--sl-surface)' }}>{cancelLabel}</button>
        <button onClick={onConfirm} style={{ flex: 1, padding: '12px 0', borderRadius: 'var(--sl-radius-xl)', cursor: 'pointer', fontSize: 14, fontWeight: 700, backgroundColor: confirmColor, color: '#fff', border: 'none' }}>{confirmLabel}</button>
      </div>
    </ModalFrame>
  );
});
