...
export default function ConsentBanner({ onAccept, onRefuse }: ConsentBannerProps) {
  return (
    <motion.div initial={{ y: 80, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 80, opacity: 0 }} transition={{ type: 'spring', stiffness: 380, damping: 36 }} role="dialog" aria-label="Consentement à la collecte de statistiques d'utilisation"
      style={{ position: 'fixed', bottom: 'calc(64px + env(safe-area-inset-bottom, 0px))', left: 0, right: 0, zIndex: (Z as Record<string, number>).helpFab + 1, margin: '0 8px 8px', borderRadius: 18, backgroundColor: 'var(--sl-card)', border: '1px solid var(--sl-border)', boxShadow: '0 8px 32px rgba(0,0,0,0.2)', padding: '14px 16px', fontFamily: 'var(--sl-font-ui)' }}>
