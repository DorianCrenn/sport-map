    ...
    <div style={{ width: 'var(--sl-sidebar-width, 380px)', display: 'flex', flexDirection: 'column', flexShrink: 0, backgroundColor: 'var(--sl-sidebar-bg)', borderLeft: '1px solid var(--sl-border)' }}>
      ...
      <div style={{ padding: '12px 14px', flexShrink: 0, backgroundColor: 'var(--sl-card)', borderBottom: '1px solid var(--sl-border)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
          ...
          <button onClick={onGeolocate} disabled={geoLoading} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, fontWeight: 600, padding: '11px 12px', borderRadius: 10, border: 'none', cursor: 'pointer', backgroundColor: 'var(--sl-blue-dim)', color: 'var(--sl-blue)', opacity: geoLoading ? 0.5 : 1, minHeight: 44 }}>
        ...
        {canAddEvent && (
          <button onClick={onAddEvent} style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '9px 0', borderRadius: 12, border: 'none', cursor: 'pointer', backgroundColor: 'var(--sl-green)', color: '#fff', fontSize: 13, fontWeight: 700 }}>
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
            Ajouter un événement
          </button>
        )}
        {pendingScores > 0 && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 10px', borderRadius: 10, marginTop: 6, backgroundColor: 'rgba(245,158,11,0.10)', border: '1px solid rgba(245,158,11,0.25)' }}>
