    ...
    <div style={{ position: 'relative', flexShrink: 0, paddingBottom: 'env(safe-area-inset-bottom, 0px)', background: 'var(--sl-bg)', display: overlayOpen ? 'none' : undefined }}>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.92, y: 8 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.92, y: 8 }} transition={{ type: 'spring', stiffness: 420, damping: 34 }} style={{ position: 'fixed', bottom: 'calc(env(safe-area-inset-bottom, 0px) + 82px)', left: isClubAdminOnly ? 'calc(50% - 148px)' : 'calc(50% - 124px)', width: isClubAdminOnly ? 296 : 248, zIndex: 1002, backgroundColor: 'var(--sl-card)', borderRadius: 20, border: '1px solid var(--sl-border-s)', boxShadow: 'var(--sl-shadow-xl)', overflow: 'hidden' }}>
              <div style={{ padding: 6 }}>
                {isClubAdminOnly ? (
                  ...
                  <>
                    <FabAction icon={<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>} label="Créer un événement" color="var(--sl-green)" onClick={() => handleFabAction('event')} />
                    <FabAction icon={<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>} label="Mes entraînements" color="#22c55e" onClick={() => handleFabAction('trainings')} />
                    {!isCoach && (
                      <>
                        <FabAction icon={<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>} label="Mon club" color="var(--sl-blue)" onClick={() => handleFabAction('clubs')} />
                        <FabAction icon={<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>} label="Importer un CSV" color="#a855f7" onClick={() => handleFabAction('csv')} />
                        <FabAction icon={<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polygon points="3 6 9 3 15 6 21 3 21 18 15 21 9 18 3 21"/></svg>} label="Voir la carte" color="#f59e0b" onClick={() => handleFabAction('map')} />
                      </>
                    )}
                  </>
                )}
      ...
      <nav style={{ display: 'flex', alignItems: 'stretch', margin: '8px 12px 12px', borderRadius: 24, background: 'var(--sl-nav-bg)', backdropFilter: 'blur(20px) saturate(160%)', WebkitBackdropFilter: 'blur(20px) saturate(160%)', boxShadow: 'var(--sl-nav-shadow)', border: '1px solid var(--sl-nav-border)', overflow: 'visible', position: 'relative' }}>
        {tabs.map((tab, tabIdx) => {
          ...
          return (
            <motion.button key={`${tab.id}-${tabIdx}`} whileTap={{ scale: 0.86 }} onClick={() => { setFabOpen(false); onTabChange(tab.id); }} data-demo={`tab-${tab.id}`} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 3, padding: '10px 4px', background: 'transparent', border: 'none', cursor: 'pointer', color, position: 'relative' }}>
...
interface FabActionProps { icon: React.ReactNode; label: string; desc?: string; color: string; onClick: () => void; [key: string]: any; }
function FabAction({ icon, label, desc, color, onClick, ...rest }: FabActionProps) {
  const [hover, setHover] = useState(false);
  const isEmoji = typeof icon === 'string';
  return (
    <button onClick={onClick} onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)} {...rest} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 12, padding: '10px 12px', borderRadius: 12, border: 'none', cursor: 'pointer', backgroundColor: hover ? 'var(--sl-hover)' : 'transparent', transition: 'background-color 0.1s' }}>
      <div style={{ width: 34, height: 34, borderRadius: 10, flexShrink: 0, backgroundColor: `${color}18`, display: 'flex', alignItems: 'center', justifyContent: 'center', color, fontSize: isEmoji ? 17 : undefined }}>{icon}</div>
