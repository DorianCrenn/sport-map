    ...
    return (
      <div style={{ borderTop: '1px solid var(--sl-border)', paddingTop: 12, marginTop: 4 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          {totalSeats > 0 ? (
            <span style={{ fontSize: 11, fontWeight: 700, padding: '2px 8px', borderRadius: 'var(--sl-radius-full)', backgroundColor: 'rgba(34,217,106,0.12)', color: 'var(--sl-green)' }}>{totalSeats} place{totalSeats > 1 ? 's' : ''} libre{totalSeats > 1 ? 's' : ''}</span>
          ) : (
            <span style={{ fontSize: 11, padding: '2px 8px', borderRadius: 'var(--sl-radius-full)', backgroundColor: 'rgba(239,68,68,0.1)', color: '#ef4444', fontWeight: 600 }}>Complet</span>
          )}
        </div>
      </div>
    );
    ...
    <>
      <div style={{ borderTop: '1px solid var(--sl-border)', paddingTop: 16, marginTop: 8 }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 14 }}>
          <div>
            <div style={{ fontSize: 14, fontWeight: 800, color: 'var(--sl-t1)', display: 'flex', alignItems: 'center', gap: 6 }}>
              🚗 Covoiturages
              {activeRides.length > 0 && <span style={{ fontSize: 11, fontWeight: 700, padding: '2px 7px', borderRadius: 'var(--sl-radius-full)', backgroundColor: 'var(--sl-surface)', color: 'var(--sl-t3)', border: '1px solid var(--sl-border)' }}>{activeRides.length}</span>}
            </div>
          ...
          {currentUser && !myRide && (
            <motion.button whileTap={{ scale: 0.96 }} onClick={() => setShowCreate(true)} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '8px 14px', borderRadius: 'var(--sl-radius-xl)', cursor: 'pointer', fontSize: 12, fontWeight: 700, border: 'none', flexShrink: 0, backgroundColor: 'var(--sl-green)', color: '#fff', boxShadow: '0 2px 8px rgba(34,217,106,0.3)' }}>
              + Proposer
            </motion.button>
          )}
        </div>
        ...
        {!loading && activeRides.length === 0 && (
          <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} style={{ borderRadius: 'var(--sl-radius-2xl)', padding: '24px 16px', textAlign: 'center', backgroundColor: 'var(--sl-surface)', border: '1px dashed var(--sl-border)' }}>
            ...
            {currentUser && (
              <motion.button whileTap={{ scale: 0.97 }} onClick={() => setShowCreate(true)} style={{ padding: '10px 22px', borderRadius: 'var(--sl-radius-xl)', cursor: 'pointer', fontSize: 13, fontWeight: 700, backgroundColor: 'var(--sl-green)', color: '#fff', border: 'none', boxShadow: '0 2px 10px rgba(34,217,106,0.3)' }}>
                Proposer un trajet
              </motion.button>
            )}
          </motion.div>
