...
export default function RideCard({
  ride, currentUser,
  onJoin, onAccept, onRefuse, onCancel, onCancelRequest,
  compact = false,
}) {
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
        <div style={{
          ...
          backgroundColor: isDriver ? 'rgba(34,217,106,0.18)' : 'var(--sl-card)',
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          fontSize: 13, fontWeight: 800, color: isDriver ? 'var(--sl-green)' : 'var(--sl-t2)',
        }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            {isDriver && (
              <span style={{ fontSize: 9, fontWeight: 700, padding: '1px 6px', borderRadius: 999, backgroundColor: 'rgba(34,217,106,0.15)', color: 'var(--sl-green)' }}>VOUS</span>
            )}
            {isCancelled && (
              <span style={{ fontSize: 9, fontWeight: 700, padding: '1px 6px', borderRadius: 999, backgroundColor: 'rgba(100,116,139,0.15)', color: '#64748b' }}>ANNULÉ</span>
            )}
          </div>
        ...
        <div style={{
          ...
          backgroundColor: isFull ? 'rgba(239,68,68,0.1)' : isCancelled ? 'var(--sl-surface)' : 'rgba(34,217,106,0.1)',
          color: isFull ? '#ef4444' : isCancelled ? 'var(--sl-t3)' : 'var(--sl-green)',
        }}>
          ...
          <span style={{ fontSize: 8, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.04em' }}>
            {ride.availableSeatsLeft !== 1 ? 'places' : 'place'}
          </span>
        </div>
      </div>

      {/* Tags: equipment + detour */}
      {!compact && (ride.acceptedEquipment.length > 0 || DETOUR_LABELS[ride.detourFlexibility]) && (
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginBottom: 10 }}>
          {ride.acceptedEquipment.map(e => (
            <span key={e} style={{ fontSize: 10, padding: '2px 8px', borderRadius: 999, backgroundColor: 'var(--sl-card)', border: '1px solid var(--sl-border)', color: 'var(--sl-t2)' }}>
              {EQUIP_LABELS[e] ?? e}
            </span>
          ))}
          {DETOUR_LABELS[ride.detourFlexibility] && (
            <span style={{ fontSize: 10, padding: '2px 8px', borderRadius: 999, backgroundColor: 'rgba(59,130,246,0.08)', border: '1px solid rgba(59,130,246,0.2)', color: '#3b82f6' }}>
              {DETOUR_LABELS[ride.detourFlexibility]}
            </span>
          )}
        </div>
      ...
      {/* Notes */}
      {!compact && ride.notes && (
        <p style={{ fontSize: 12, color: 'var(--sl-t2)', margin: '0 0 10px', lineHeight: 1.5, borderLeft: '2px solid var(--sl-border)', paddingLeft: 10 }}>
          {ride.notes}
        </p>
      )}
      ...
      {isDriver && pending.length > 0 && (
        <div style={{ marginBottom: 10 }}>
          <div style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--sl-t3)', marginBottom: 6 }}>
          ...
          {pending.map(req => (
            <div key={req.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 10px', borderRadius: 10, backgroundColor: 'var(--sl-card)', marginBottom: 4 }}>
              <div style={{ width: 28, height: 28, borderRadius: 8, backgroundColor: 'var(--sl-surface)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 800, color: 'var(--sl-t2)', flexShrink: 0 }}>
                {avatar(req.passengerName)}
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
      ...
      {isDriver && accepted.length > 0 && (
        <div style={{ marginBottom: 10 }}>
          <div style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--sl-t3)', marginBottom: 6 }}>
            {accepted.length} passager{accepted.length > 1 ? 's' : ''} confirmé{accepted.length > 1 ? 's' : ''}
          </div>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
            {accepted.map(req => (
              <span key={req.id} style={{ fontSize: 11, padding: '4px 10px', borderRadius: 999, backgroundColor: 'rgba(34,217,106,0.1)', color: 'var(--sl-green)', fontWeight: 600 }}>
                ✓ {req.passengerName}
              </span>
            ))}
          </div>
      ...
      {!isDriver && currentUser && !isCancelled && (
        <div style={{ marginTop: 4 }}>
          {!myRequest && !isFull && (
            <motion.button
              ...
              style={{
                ...
                border: 'none', backgroundColor: 'var(--sl-green)', color: '#fff',
                fontSize: 13, fontWeight: 700, boxShadow: '0 2px 10px rgba(34,217,106,0.3)',
              }}
            >
              Rejoindre ce trajet
            </motion.button>
          ...
          {!myRequest && isFull && (
            <div style={{ textAlign: 'center', fontSize: 12, fontWeight: 600, color: '#ef4444', padding: '8px 0' }}>Trajet complet</div>
          )}
          {myRequest && (
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '4px 2px' }}>
              <span style={{ fontSize: 12, fontWeight: 700, color: STATUS_COLORS[myRequest.status]?.text }}>
                {STATUS_COLORS[myRequest.status]?.label ?? myRequest.status}
              </span>
        ...
        <div style={{ marginTop: 8 }}>
          {!confirmCancel ? (
            <button
              ...
              style={{ width: '100%', padding: '9px 0', borderRadius: 12, cursor: 'pointer', fontSize: 12, fontWeight: 600, border: '1px solid var(--sl-border)', backgroundColor: 'transparent', color: 'var(--sl-t3)' }}
            >
              Annuler mon trajet
            </button>
          ) : (
            <div style={{ display: 'flex', gap: 8 }}>
              <button onClick={() => setConfirmCancel(false)} style={{ flex: 1, padding: '9px 0', borderRadius: 12, cursor: 'pointer', fontSize: 12, fontWeight: 600, border: '1px solid var(--sl-border)', backgroundColor: 'transparent', color: 'var(--sl-t2)' }}>
                Garder
              </button>
              <button
                onClick={() => { onCancel(); setConfirmCancel(false); }}
                style={{ flex: 1, padding: '9px 0', borderRadius: 12, cursor: 'pointer', fontSize: 12, fontWeight: 700, border: 'none', backgroundColor: 'rgba(239,68,68,0.12)', color: '#ef4444' }}
              >
                Confirmer
              </button>
            </div>
