  ...
  return (
    ...
    >
      <motion.div
        ...
        style={{ width: '100%', maxWidth: 400, borderRadius: 'var(--sl-radius-4xl)', backgroundColor: 'var(--sl-card)', border: '1px solid var(--sl-border)', maxHeight: '88dvh', overflowY: 'auto', overflowX: 'hidden', overscrollBehavior: 'contain' }}
      ...
      >
        <div style={{ padding: '20px 20px 14px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 2 }}>
              <span style={{ fontSize: 11, fontWeight: 700, padding: '2px 8px', borderRadius: 'var(--sl-radius-full)', backgroundColor: 'rgba(34,217,106,0.12)', color: 'var(--sl-green)' }}>{ride.availableSeatsLeft} place{ride.availableSeatsLeft !== 1 ? 's' : ''} libre{ride.availableSeatsLeft !== 1 ? 's' : ''}</span>
            </div>
          </div>
        </div>
        <form onSubmit={handleSubmit}>
          <div style={{ padding: '0 20px 16px' }}>
            ...
            <textarea value={message} onChange={e => setMessage(e.target.value)} placeholder="ex. Je serai devant l'entrée principale, je préférerais partir à 14h…" rows={3} style={{ width: '100%', boxSizing: 'border-box', borderRadius: 'var(--sl-radius-xl)', padding: '10px 12px', fontSize: 13, backgroundColor: 'var(--sl-surface)', border: '1px solid var(--sl-border)', color: 'var(--sl-t1)', resize: 'none', outline: 'none' }} />
          ...
          <div style={{ display: 'flex', gap: 8, padding: '0 20px 20px' }}>
            <button type="button" onClick={onClose} style={{ flex: 1, padding: '12px 0', borderRadius: 'var(--sl-radius-2xl)', border: '1px solid var(--sl-border-s)', backgroundColor: 'var(--sl-surface)', color: 'var(--sl-t2)', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>Annuler</button>
            <button type="submit" disabled={saving} style={{ flex: 2, padding: '12px 0', borderRadius: 'var(--sl-radius-2xl)', border: 'none', backgroundColor: saving ? 'var(--sl-surface)' : 'var(--sl-green)', color: saving ? 'var(--sl-t3)' : '#fff', fontSize: 14, fontWeight: 700, cursor: saving ? 'default' : 'pointer', boxShadow: saving ? 'none' : '0 4px 12px rgba(34,217,106,0.3)' }}>{saving ? 'Envoi…' : 'Demander à rejoindre'}</button>
          </div>
        </form>
      </motion.div>
    </motion.div>
