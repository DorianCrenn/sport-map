...
export default function CreateRideModal({ event, onSave, onClose }) {
    ...
    >
      <motion.div
        ...
        style={{
          ...
          backgroundColor: 'var(--sl-card)', borderRadius: '22px 22px 0 0',
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 18px 14px', borderBottom: '1px solid var(--sl-border)', flexShrink: 0 }}>
          <div>
            ...
            <div style={{ fontSize: 11, color: 'var(--sl-t3)', marginTop: 3 }}>
              {event.title} · {dateLabel} à {timeLabel}
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <form id="create-ride-form" onSubmit={handleSubmit} style={{ flex: 1, overflowY: 'auto', padding: '18px 18px 8px', display: 'flex', flexDirection: 'column', gap: 16, overscrollBehavior: 'contain' }}>
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              {[1, 2, 3, 4, 5, 6].map(n => {
                ...
                return (
                  <button
                    key={n} type="button"
                    onClick={() => set('availableSeats', n)}
                    style={{
                      flex: 1, padding: '10px 0', borderRadius: 12, cursor: 'pointer',
                      border: `2px solid ${sel ? 'var(--sl-green)' : 'var(--sl-border)'}`,
                      backgroundColor: sel ? 'rgba(34,217,106,0.12)' : 'var(--sl-surface)',
                      color: sel ? 'var(--sl-green)' : 'var(--sl-t2)',
                      fontSize: 14, fontWeight: 800, transition: 'all 0.12s',
                    }}
                  >
                    {n === 6 ? '6+' : n}
                  </button>
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      border: `1.5px solid ${sel ? 'var(--sl-green)' : 'var(--sl-border)'}`,
                      backgroundColor: sel ? 'rgba(34,217,106,0.1)' : 'var(--sl-surface)',
                      color: sel ? 'var(--sl-green)' : 'var(--sl-t2)',
                      fontSize: 12, fontWeight: sel ? 700 : 500, transition: 'all 0.12s',
                    }}
                  >
                    {opt.label}
                  </button>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 6 }}>
              {DETOUR_OPTIONS.map(opt => {
                ...
                return (
                  <button
                    key={opt.key} type="button"
                    onClick={() => set('detourFlexibility', opt.key)}
                    style={{
                      padding: '9px 6px', borderRadius: 10, cursor: 'pointer', textAlign: 'center',
                      border: `1.5px solid ${sel ? '#3b82f6' : 'var(--sl-border)'}`,
                      backgroundColor: sel ? 'rgba(59,130,246,0.1)' : 'var(--sl-surface)',
          ...
          <Field label="Notes (optionnel)">
            <textarea
              ...
              placeholder="Départ depuis le parking Leclerc, retour prévu après le match…"
              rows={2}
        ...
        <div style={{ flexShrink: 0, padding: '14px 18px 20px', borderTop: '1px solid var(--sl-border)', display: 'flex', gap: 10 }}>
          <button
            ...
            style={{ flex: 1, padding: '13px 0', borderRadius: 14, border: '1px solid var(--sl-border-s)', color: 'var(--sl-t2)', backgroundColor: 'var(--sl-surface)', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}
          >
            Annuler
          </button>
          <button
            type="submit" form="create-ride-form" disabled={saving}
            style={{
              flex: 2, padding: '13px 0', borderRadius: 14, border: 'none',
              backgroundColor: saving ? 'var(--sl-surface)' : 'var(--sl-green)',
              color: saving ? 'var(--sl-t3)' : '#fff',
              fontSize: 14, fontWeight: 700, cursor: saving ? 'default' : 'pointer',
              boxShadow: saving ? 'none' : '0 4px 14px rgba(34,217,106,0.35)',
