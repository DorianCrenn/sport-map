...
interface TennisSetsInputProps {
  setsPerMatch?: number;
  value?: SetsValue;
  onChange: (value: SetsValue) => void;
}

export default function TennisSetsInput({ setsPerMatch = 3, value, onChange }: TennisSetsInputProps) {
  const sets   = value?.sets ?? [];
  const toWin  = Math.ceil(setsPerMatch / 2);

  function setScore(idx: number, side: string, v: string) {
  ...
  function setTiebreak(idx: number, side: string, v: string) {
    const n    = v === '' ? 0 : Math.max(0, parseInt(v, 10) || 0);
    const next = [...sets];
    next[idx]  = { ...next[idx], tiebreak: { ...(next[idx]?.tiebreak ?? { home: 0, away: 0 }), [side]: n } };
    onChange(computeFromSets(next));
  }

  function computeFromSets(ss: TennisSet[]): SetsValue {
    let homeWins = 0, awayWins = 0;
    for (const s of ss) {
      const h = s.home ?? 0, a = s.away ?? 0;
      if (h === 0 && a === 0) continue;
      const winner = (h === 7 && a === 6) ? 'home' : (h === 6 && a === 7) ? 'away'
    ...
    return { sets: ss, sets_won: { home: homeWins, away: awayWins }, winner_side };
  }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      ...
      {Array.from({ length: displayCount }, (_, i) => {
        ...
        return (
          <div key={i} style={{ display: 'grid', gridTemplateColumns: '28px 1fr auto 1fr', gap: 4, alignItems: 'center' }}>
            <span style={{ fontSize: 9, fontWeight: 700, color: homeWon ? '#22c55e' : awayWon ? '#ef4444' : 'var(--sl-t3)', textAlign: 'center' }}>S{i + 1}</span>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
              <input type="number" inputMode="numeric" min="0" max="7" value={s.home ?? ''} onChange={e => setScore(i, 'home', e.target.value)} style={{ ...numSt, borderColor: homeWon ? '#22c55e' : 'var(--sl-border-s)' }} />
              {isTiebreak && <input type="number" inputMode="numeric" min="0" max="20" value={s.tiebreak?.home ?? ''} onChange={e => setTiebreak(i, 'home', e.target.value)} style={tbSt} placeholder="TB" />}
            </div>
            <span style={{ fontWeight: 700, color: 'var(--sl-t3)', textAlign: 'center' }}>—</span>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
              <input type="number" inputMode="numeric" min="0" max="7" value={s.away ?? ''} onChange={e => setScore(i, 'away', e.target.value)} style={{ ...numSt, borderColor: awayWon ? '#22c55e' : 'var(--sl-border-s)' }} />
              {isTiebreak && <input type="number" inputMode="numeric" min="0" max="20" value={s.tiebreak?.away ?? ''} onChange={e => setTiebreak(i, 'away', e.target.value)} style={tbSt} placeholder="TB" />}
            </div>
          </div>
        );
      })}
      ...
      {(setsWonHome > 0 || setsWonAway > 0) && (
        <div style={{ display: 'flex', justifyContent: 'center', gap: 8, marginTop: 4 }}>
          ...
          <span style={{ fontSize: 12, fontWeight: 800, color: setsWonHome > setsWonAway ? '#22c55e' : 'var(--sl-t1)' }}>{setsWonHome}</span>
          <span style={{ color: 'var(--sl-t3)' }}>—</span>
          <span style={{ fontSize: 12, fontWeight: 800, color: setsWonAway > setsWonHome ? '#22c55e' : 'var(--sl-t1)' }}>{setsWonAway}</span>
        </div>
      )}
    </div>
  );
