...
interface TennisEncountersManagerProps {
    ...
    <div style={{ borderTop: '1px solid var(--sl-border)', paddingTop: 14 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {(config.encounterTypes ?? []).map((type: any) => {
          ...
          return (
            <EncounterCard key={type.key} encounter={enc} encounterType={type} clubId={event?.clubId} setsPerMatch={config.setsPerMatch ?? 3} onSave={handleEncounterSave} />
          );
        })}
      </div>
