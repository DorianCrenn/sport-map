...
interface ScoreConfig {
  homeLabel: string;
  awayLabel: string;
  fields?: { optional?: Array<{ group: string }> };
}

interface SimpleScoreInputsProps {
  config: ScoreConfig;
  value: Record<string, any>;
  onChange: (value: Record<string, any>) => void;
}

export default function SimpleScoreInputs({ config, value, onChange }: SimpleScoreInputsProps) {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12 }}>
        <div>
          ...
          <input type="number" inputMode="numeric" min="0" max="99" value={value?.home ?? ''} onChange={e => set('home', e.target.value)} style={numStyle} />
        </div>
        <span style={{ fontWeight: 900, fontSize: 22, color: 'var(--sl-t3)', marginTop: 18 }}>—</span>
        <div>
          <label style={lbl}>{config.awayLabel}</label>
          <input type="number" inputMode="numeric" min="0" max="99" value={value?.away ?? ''} onChange={e => set('away', e.target.value)} style={numStyle} />
        </div>
      </div>
      ...
      {showExtra && hasExtraTime && (
        <div>
          <div style={{ fontSize: 10, fontWeight: 700, color: 'var(--sl-t3)', textTransform: 'uppercase', letterSpacing: '0.07em', textAlign: 'center', marginBottom: 8 }}>Prolongation</div>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10 }}>
            <input type="number" inputMode="numeric" min="0" max="20" value={value?.et_home ?? ''} onChange={e => set('et_home', e.target.value)} placeholder="0" style={optStyle} />
            <span style={{ color: 'var(--sl-t3)', fontWeight: 700 }}>—</span>
            <input type="number" inputMode="numeric" min="0" max="20" value={value?.et_away ?? ''} onChange={e => set('et_away', e.target.value)} placeholder="0" style={optStyle} />
          </div>
        </div>
      )}
      ...
      {showExtra && hasPenalties && isDrawAfterET && (
        <div>
          <div style={{ fontSize: 10, fontWeight: 700, color: 'var(--sl-t3)', textTransform: 'uppercase', letterSpacing: '0.07em', textAlign: 'center', marginBottom: 8 }}>Tirs au but</div>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10 }}>
            <input type="number" inputMode="numeric" min="0" max="20" value={value?.pen_home ?? ''} onChange={e => set('pen_home', e.target.value)} placeholder="0" style={optStyle} />
            <span style={{ color: 'var(--sl-t3)', fontWeight: 700 }}>—</span>
            <input type="number" inputMode="numeric" min="0" max="20" value={value?.pen_away ?? ''} onChange={e => set('pen_away', e.target.value)} placeholder="0" style={optStyle} />
          </div>
        </div>
      )}
    </div>
