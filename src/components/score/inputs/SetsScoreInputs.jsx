...
/**
 * Saisie par sets : score (sets gagnés) + détail optionnel par set.
 * Sports : volleyball.
 */
export default function SetsScoreInputs({ config, value, onChange }) {
  function setMain(key, v) {
    const n = v === '' ? undefined : parseInt(v, 10);
    onChange({ ...value, [key]: isNaN(n) ? undefined : n });
  }

  function setSetScore(idx, side, v) {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12 }}>
        <div>
          ...
          <input type="number" inputMode="numeric" min="0" max={config.setsToWin ?? 5}
            value={value?.sets_home ?? ''} onChange={e => setMain('sets_home', e.target.value)} style={bigStyle} />
        </div>
        <span style={{ fontWeight: 900, fontSize: 22, color: 'var(--sl-t3)', marginTop: 18 }}>—</span>
        <div>
          <label style={lbl}>{config.awayLabel} (sets)</label>
          <input type="number" inputMode="numeric" min="0" max={config.setsToWin ?? 5}
            value={value?.sets_away ?? ''} onChange={e => setMain('sets_away', e.target.value)} style={bigStyle} />
        </div>
      </div>
      ...
      {showSetDetail && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          <div style={{ fontSize: 10, fontWeight: 700, color: 'var(--sl-t3)', textTransform: 'uppercase', letterSpacing: '0.07em', textAlign: 'center' }}>
            Détail par set
          </div>
          {Array.from({ length: totalSets }, (_, i) => {
            const s = sets[i] ?? { set: i + 1, home: 0, away: 0 };
            const isTiebreak = i === config.maxSets - 1;
            ...
            return (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, justifyContent: 'center' }}>
                ...
                <input type="number" inputMode="numeric" min="0" max={maxPts + 10}
                  value={s.home} onChange={e => setSetScore(i, 'home', e.target.value)} style={numStyle} />
                <span style={{ color: 'var(--sl-t3)', fontWeight: 700 }}>—</span>
                <input type="number" inputMode="numeric" min="0" max={maxPts + 10}
                  value={s.away} onChange={e => setSetScore(i, 'away', e.target.value)} style={numStyle} />
              </div>
            );
          })}
        </div>
