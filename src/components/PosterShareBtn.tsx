...
export default function PosterShareBtn({ event }: PosterShareBtnProps) {
  const [phase, setPhase] = useState<'idle' | 'rendering' | 'exporting'>('idle');
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        if ((navigator as any).canShare?.({ files: [file] })) { await navigator.share({ files: [file], title: event?.title ?? 'SportLink' }); }
        else { const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = `affiche-${(event?.title ?? 'match').replace(/\s+/g, '-').toLowerCase()}.png`; a.click(); URL.revokeObjectURL(url); }
  ...
  return (
    <>
      ...
      <button onClick={e => { e.stopPropagation(); if (!busy) setPhase('rendering'); }} disabled={busy} aria-label="Partager l'affiche" title="Partager l'affiche"
