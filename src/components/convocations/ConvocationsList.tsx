  ...
  return (
    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden">
      <div style={{ paddingTop: 10, borderTop: '1px solid var(--sl-border)', marginTop: 8 }}>
        ...
        <button onClick={() => setSelected('driving')} style={{ width: '100%', marginBottom: 6, padding: '10px 12px', borderRadius: 10, cursor: 'pointer', textAlign: 'left', border: `1.5px solid ${selected === 'driving' ? '#22c55e' : 'var(--sl-border)'}`, backgroundColor: selected === 'driving' ? 'rgba(34,197,94,0.07)' : 'transparent', display: 'flex', alignItems: 'center', gap: 10, transition: 'all 0.15s' }}>
          ...
          {selected === 'driving' && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
              {[1, 2, 3, 4].map(n => (
                <button key={n} onClick={e => { e.stopPropagation(); setDriverSeats(n); }} style={{ width: 36, height: 36, borderRadius: 10, cursor: 'pointer', border: `1.5px solid ${driverSeats === n ? '#22c55e' : 'var(--sl-border)'}`, backgroundColor: driverSeats === n ? 'rgba(34,197,94,0.15)' : 'transparent', color: driverSeats === n ? '#22c55e' : 'var(--sl-t2)', fontSize: 13, fontWeight: 800 }}>{n}</button>
              ))}
              <span style={{ fontSize: 10, color: 'var(--sl-t3)', marginLeft: 2 }}>places</span>
            </div>
          )}
        </button>
        <button onClick={() => setSelected('passenger')} style={{ width: '100%', marginBottom: 6, padding: '10px 12px', borderRadius: 10, cursor: 'pointer', textAlign: 'left', border: `1.5px solid ${selected === 'passenger' ? '#6366f1' : 'var(--sl-border)'}`, backgroundColor: selected === 'passenger' ? 'rgba(99,102,241,0.07)' : 'transparent', display: 'flex', alignItems: 'center', gap: 10, transition: 'all 0.15s' }}>
          <span style={{ fontSize: 18, lineHeight: 1 }}>🙋</span>
          <div>
        ...
        <button onClick={() => setSelected('own_means')} style={{ width: '100%', marginBottom: 10, padding: '10px 12px', borderRadius: 10, cursor: 'pointer', textAlign: 'left', border: `1.5px solid ${selected === 'own_means' ? '#94a3b8' : 'var(--sl-border)'}`, backgroundColor: selected === 'own_means' ? 'rgba(148,163,184,0.07)' : 'transparent', display: 'flex', alignItems: 'center', gap: 10, transition: 'all 0.15s' }}>
          <span style={{ fontSize: 18, lineHeight: 1 }}>🏡</span>
          <div>
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          <button onClick={onBack} style={{ flex: 1, padding: '9px 0', borderRadius: 9, cursor: 'pointer', border: '1.5px solid var(--sl-border)', backgroundColor: 'transparent', color: 'var(--sl-t3)', fontSize: 12, fontWeight: 600 }}>Retour</button>
          <button onClick={confirm} disabled={!selected} style={{ flex: 2, padding: '9px 0', borderRadius: 9, cursor: 'pointer', border: 'none', backgroundColor: selected ? '#22c55e' : 'var(--sl-card-hi)', color: selected ? '#000' : 'var(--sl-t3)', fontSize: 13, fontWeight: 700, transition: 'all 0.15s' }}>Confirmer</button>
        </div>
      </div>
    </motion.div>
  );
...
function ConvocationCard({ conv, onRespond }: ConvocationCardProps) {
  ...
  return (
    <motion.div layout initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }} style={{ borderRadius: 14, border: `1.5px solid ${statusCfg.color}40`, backgroundColor: 'var(--sl-card)', overflow: 'hidden' }}>
      ...
      <div style={{ padding: '11px 14px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
          ...
          <span style={{ fontSize: 9, fontWeight: 700, padding: '2px 7px', borderRadius: 20, backgroundColor: statusCfg.bg, color: statusCfg.color }}>{statusCfg.label}</span>
        </div>
        ...
        {conv.status === 'pending' && !awaitingTransport && (
          <div data-demo="convocation-respond" style={{ display: 'flex', gap: 5 }}>
            <button onClick={() => setAwaitingTransport(true)} style={{ flex: 1, padding: '10px 6px', minHeight: 44, borderRadius: 9, cursor: 'pointer', border: '1.5px solid var(--sl-border)', backgroundColor: 'transparent', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 3, transition: 'all 0.15s' }}>
            ...
            {[
              ...
              <button key={btn.status} onClick={() => onRespond(conv.id, btn.status)} style={{ flex: 1, padding: '10px 6px', minHeight: 44, borderRadius: 9, cursor: 'pointer', border: '1.5px solid var(--sl-border)', backgroundColor: 'transparent', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 3, transition: 'all 0.15s' }}>
    ...
    <div style={{ marginBottom: 12 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          {pending.length > 0 && <span style={{ fontSize: 9, fontWeight: 800, color: '#f59e0b', backgroundColor: 'rgba(245,158,11,0.12)', padding: '2px 6px', borderRadius: 20 }}>{pending.length} en attente</span>}
        </div>
