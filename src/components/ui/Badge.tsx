...
export default function Badge({ color, label, size = 'sm', className = '' }: BadgeProps) {
  ...
  return (
    <span className={className} style={{ display: 'inline-flex', alignItems: 'center', padding, borderRadius: 6, fontSize, fontWeight: 700, color, backgroundColor: `${color}20`, flexShrink: 0, whiteSpace: 'nowrap' }}>
      {label}
    </span>
  );
}
