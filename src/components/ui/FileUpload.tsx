...
export default function FileUpload({ clubId, value, onChange, label = 'Choisir un fichier', hint, dark = false }: FileUploadProps) {
  ...
  const inp: React.CSSProperties = { padding: '7px 10px', borderRadius: 8, fontSize: 12, border: '1px solid var(--sl-border-s)', backgroundColor: 'var(--sl-surface)', color: 'var(--sl-t1)', outline: 'none', flex: 1, boxSizing: 'border-box' };

  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
        <button type="button" onClick={() => inputRef.current?.click()} disabled={uploading} style={{ padding: '7px 12px', borderRadius: 8, fontSize: 12, fontWeight: 600, border: '1.5px dashed var(--sl-border-s)', cursor: uploading ? 'wait' : 'pointer', backgroundColor: value ? 'rgba(99,102,241,0.08)' : 'var(--sl-surface)', color: value ? '#6366f1' : 'var(--sl-t2)', whiteSpace: 'nowrap', flexShrink: 0 }}>
      ...
      {value && (
        <div style={{ padding: '8px 10px', borderRadius: 8, backgroundColor: dark ? '#111827' : 'var(--sl-surface)', border: '1px solid var(--sl-border)', display: 'flex', alignItems: 'center', gap: 8 }}>
          <img src={value} alt="Aperçu" style={{ height: 32, maxWidth: 80, objectFit: 'contain', borderRadius: 4 }} onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} />
