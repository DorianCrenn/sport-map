...
function TeamCircle({ name, logo, accent, isHome, blockId, bStyle = {} }) {
  return (
    <div data-block={blockId} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 9, flex: 1, ...bStyle }}>
      <div style={{
        ...
        {logo
          ...
          : <span style={{ fontSize: 19, fontWeight: 700, color: isHome ? accent : 'rgba(255,255,255,0.55)' }}>{initials(name)}</span>
        }
      </div>
      <span style={{
...
function MetaIcon({ type }) {
  const icons = {
    date: <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>,
    ...
    pin: <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>,
  };
  return icons[type] || null;
}

export default function TplEditorial({ event, homeTeam, awayTeam, championship, tagline, accentColor = '#D4AF37', bgImage, format = 'story', transforms = {} as any }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ position: 'relative', zIndex: 10, display: 'flex', flexDirection: 'column', height: '100%' }}>
        ...
        <div data-block="title" style={{ marginBottom: format === 'story' ? 22 : 16, ...tr('title') }}>
          <Rule accent={a} />
          <div style={{ textAlign: 'center', lineHeight: 0.9, padding: '10px 0' }}>
            <div style={{ fontSize: format === 'story' ? 84 : 70, fontWeight: 200, color: 'rgba(255,255,255,0.90)', letterSpacing: '-0.04em' }}>Match</div>
            <div style={{ fontSize: format === 'story' ? 92 : 76, fontWeight: 300, color: a, letterSpacing: '-0.04em', marginTop: -8 }}>Day</div>
          </div>
          <Rule accent={a} />
        </div>
        ...
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', width: '100%' }}>
            <TeamCircle name={homeName} logo={homeTeam?.logo} accent={a} isHome={true} blockId="home-team" bStyle={tr('home-team')} />
            ...
            <TeamCircle name={awayName} logo={awayTeam?.logo} accent={a} isHome={false} blockId="away-team" bStyle={tr('away-team')} />
          </div>
        </div>
