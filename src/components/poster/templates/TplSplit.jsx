...
function LogoBox({ name, logo, isHome, accent, size }) {
  const sz = size || 86;
    ...
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
      <div style={{
        ...
        {logo
          ...
          : <span style={{ fontSize: 22, fontWeight: 900, color: isHome ? accent : 'rgba(255,255,255,0.4)', letterSpacing: '-0.02em' }}>{initials(name)}</span>
        }
      </div>
      <div style={{ textAlign: 'center', maxWidth: 110 }}>
        <span style={{
          display: 'block',
          fontSize: scaledFs(name, 11, 12, 8),
          fontWeight: 800,
          color: isHome ? 'white' : 'rgba(255,255,255,0.65)',
...
export default function TplSplit({ event, homeTeam, awayTeam, championship, tagline, accentColor = '#3b82f6', bgImage, format = 'story' }) {
    ...
    <div style={{
      ...
      <div style={{ position: 'relative', zIndex: 10, display: 'flex', flexDirection: 'column', height: '100%', padding: '22px 24px 20px' }}>
        ...
        <div style={{ flex: 1, display: 'flex', alignItems: 'center' }}>
          <LogoBox name={homeName} logo={homeTeam?.logo} isHome={true} accent={a} size={isStory ? 90 : 72} />
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flexShrink: 0, gap: 6, padding: '0 12px' }}>
            ...
            <div style={{
              ...
              backgroundColor: `${a}18`, border: `1.5px solid ${a}55`,
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              boxShadow: `0 0 18px ${a}25`,
            }}>
          ...
          <LogoBox name={awayName} logo={awayTeam?.logo} isHome={false} accent={a} size={isStory ? 90 : 72} />
        </div>
        ...
        {/* Meta */}
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: tagline ? 10 : 14 }}>
          {[
            ...
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
              {icon}
              <span style={{ fontSize: 10, fontWeight: 600, color: 'rgba(255,255,255,0.52)', letterSpacing: '0.03em' }}>{val}</span>
            </div>
          ))}
        </div>

        {tagline && (
          <div style={{ marginBottom: 12, padding: '7px 12px', borderRadius: 9, backgroundColor: `${a}10`, border: `1px solid ${a}25` }}>
