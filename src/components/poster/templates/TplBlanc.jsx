...
function TeamCircleLight({ name, logo, accent }) {
...
export default function TplBlanc({ event, homeTeam, awayTeam, championship, tagline, accentColor = '#B38B59', bgImage, format = 'story' }) {
  ...
  return (
    <div style={{
      ...
      {/* Masthead */}
      <div style={{
        position: 'relative', zIndex: 10, backgroundColor: DARK,
        padding: '13px 24px',
      ...
      <div style={{ position: 'relative', zIndex: 10, flex: 1, display: 'flex', flexDirection: 'column', padding: '18px 24px 18px' }}>
        ...
        <div style={{ marginBottom: 12 }}>
          ...
          <div style={{ fontSize: 10.5, fontWeight: 600, color: a, letterSpacing: '0.08em', marginTop: 8 }}>
            {champ}
          </div>
        </div>
        ...
        <div style={{ flex: 1, display: 'flex', alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', width: '100%' }}>
            <TeamCircleLight name={homeName} logo={homeTeam?.logo} accent={a} />
            ...
            <TeamCircleLight name={awayName} logo={awayTeam?.logo} accent={a} />
          </div>
        </div>
        ...
        {/* Meta - clean icon rows */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: 7, marginBottom: tagline ? 10 : 0 }}>
          {[
            {
              svg: <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke={DARK} strokeWidth="2" strokeLinecap="round" style={{ opacity: 0.55 }}><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>,
              value: `${dt.short}  ·  ${dt.time}`,
            },
            {
              svg: <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke={DARK} strokeWidth="2" strokeLinecap="round" style={{ opacity: 0.55 }}><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>,
              value: (event?.venue || event?.city || '—').slice(0, 32),
            },
          ].map(({ svg, value }, i) => (
        ...
        {tagline && (
          <div style={{ marginTop: 10 }}>
            <span style={{ fontSize: 9, fontWeight: 700, letterSpacing: '0.28em', color: a, textTransform: 'uppercase' }}>
              {tagline}
            </span>
          </div>
        )}
