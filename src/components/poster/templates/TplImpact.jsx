...
function TeamBlock({ name, logo, accent, isHome }) {
...
export default function TplImpact({ event, homeTeam, awayTeam, championship, tagline, accentColor = '#22D96A', bgImage, format = 'story' }) {
      ...
      <div style={{ position: 'relative', zIndex: 10, display: 'flex', flexDirection: 'column', height: '100%' }}>
        ...
        <div style={{ marginBottom: 12 }}>
          <span style={{
            ...
            borderLeft: `2.5px solid ${a}`, paddingLeft: 9,
          }}>
            {champ}
          </span>
        </div>
        ...
        {/* Hero title */}
        <div style={{ marginBottom: isStory ? 14 : 10 }}>
          <div style={{
            fontSize: isStory ? 112 : 92,
            fontWeight: 900, color: 'white', textTransform: 'uppercase',
            letterSpacing: '-0.04em', lineHeight: 0.87, fontStyle: 'italic',
          }}>
            MATCH
          </div>
          <div style={{
            fontSize: isStory ? 112 : 92,
            fontWeight: 900, color: a, textTransform: 'uppercase',
            letterSpacing: '-0.04em', lineHeight: 0.87, fontStyle: 'italic',
            marginTop: -6,
          }}>
            DAY
          ...
          {tagline && (
            <div style={{ fontSize: 8.5, fontWeight: 700, color: 'rgba(255,255,255,0.38)', letterSpacing: '0.22em', textTransform: 'uppercase', marginTop: 10 }}>
              {tagline}
            </div>
          )}
        </div>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <TeamBlock name={homeName} logo={homeTeam?.logo} accent={a} isHome={true} />

            {/* VS badge */}
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
              <div style={{
                width: 32, height: 32, borderRadius: '50%',
                backgroundColor: `${a}1A`, border: `1.5px solid ${a}55`,
                display: 'flex', alignItems: 'center', justifyContent: 'center',
              }}>
            ...
            <TeamBlock name={awayName} logo={awayTeam?.logo} accent={a} isHome={false} />
          </div>
        ...
        <div style={{ marginBottom: 12 }}>
          {[
            {
              svg: <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>,
              value: `${dt.short} — ${dt.time}`,
            },
            {
              svg: <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>,
              value: (event?.venue || event?.city || '—').slice(0, 30),
            },
          ].map(({ svg, value }, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 9, paddingBottom: 7 }}>
              <div style={{ width: 24, height: 24, borderRadius: 7, backgroundColor: `${a}14`, border: `1px solid ${a}25`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: a, flexShrink: 0 }}>
                {svg}
              </div>
