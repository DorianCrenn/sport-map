...
function TeamCircleLight({ name, logo, blockId, bStyle = {}, accent: _accent }: { name: any; logo: any; blockId: any; bStyle?: Record<string, any>; accent?: any }) {
  return (
...
export default function TplBlanc({ event, homeTeam, awayTeam, championship, tagline, accentColor = '#B38B59', bgImage, format = 'story', transforms = {} as any }) {
      ...
      <div style={{ position: 'relative', zIndex: 10, flex: 1, display: 'flex', flexDirection: 'column', padding: '18px 24px 18px' }}>
        ...
        <div data-block="meta" style={{ display: 'flex', flexDirection: 'column', gap: 7, marginBottom: tagline ? 10 : 0, ...tr('meta') }}>
          {[
            ...
            { svg: <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke={DARK} strokeWidth="2" strokeLinecap="round" style={{ opacity: 0.55 }}><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>, value: event?.venue || event?.city || '—' },
          ].map(({ svg, value }, i) => (
