...
const PLAN_ROWS: { key: string; label: string }[] = [
...
function fmtQuota(value: number | null | undefined, unit = ''): string {
...
interface PlanCardProps {
  ...
  quotas: Record<string, any> | null | undefined;
  canUseFeature: (feature: string, planId: string) => boolean;
            ...
            <div key={key} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              {allowed ? (
                <div style={{ width: 16, height: 16, borderRadius: '50%', flexShrink: 0, backgroundColor: `${color}20`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="3.5" strokeLinecap="round"><polyline points="20 6 9 17 4 12"/></svg>
                </div>
              ) : (
                <div style={{ width: 16, height: 16, borderRadius: '50%', flexShrink: 0, backgroundColor: 'var(--sl-surface)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <svg width="7" height="7" viewBox="0 0 24 24" fill="none" stroke="var(--sl-t3)" strokeWidth="3" strokeLinecap="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
                </div>
              )}
              <span style={{ fontSize: 12, fontWeight: allowed ? 500 : 400, color: allowed ? 'var(--sl-t1)' : 'var(--sl-t3)', opacity: allowed ? 1 : 0.55 }}>{label}</span>
            </div>
          );
        })}
      </div>
...
export default function PlansSection({ onCta }: PlansSectionProps) {
  ...
  const order: string[] = loading ? ['free', 'starter', 'pro', 'elite'] : planOrder;
  ...
  return (
    <section style={{ padding: '48px 0 0' }}>
      ...
      <div className="md:hidden" style={{ display: 'flex', gap: 14, overflowX: 'auto', scrollSnapType: 'x mandatory', scrollbarWidth: 'none', padding: '0 20px 20px', WebkitOverflowScrolling: 'touch' } as React.CSSProperties}>
        {order.map((planId, i) => meta[planId] && <PlanCard key={planId} planId={planId} index={i} meta={meta[planId]} quotas={getQuotas(planId)} canUseFeature={canUseFeature} onCta={handleCta} />)}
      ...
      <motion.p initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: 0.4 }} style={{ textAlign: 'center', fontSize: 12, color: 'var(--sl-t3)', padding: '8px 20px 0', margin: 0 }}>
