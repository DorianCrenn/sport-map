...
interface AnnouncementCardProps { ann: Ann; variant?: 'full' | 'center' | 'dashboard'; isRead?: boolean; onRead?: (id: string | number) => void; onDelete?: (id: string | number) => Promise<void> | void; isLast?: boolean; }

export default function AnnouncementCard({ ann, variant = 'full', isRead = false, onRead, onDelete, isLast = false }: AnnouncementCardProps) {
  ...
  if (variant === 'dashboard') {
    async function handleDelete() { setDeleting(true); await onDelete?.(ann.id); setDeleting(false); setConfirmDelete(false); }
    return (
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '10px 16px', borderBottom: isLast ? 'none' : '1px solid var(--sl-border)' }}>
  ...
  if (variant === 'center') {
    return (
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} onClick={() => !isRead && onRead?.(ann.id)} style={{ borderRadius: 16, padding: 14, marginBottom: 10, backgroundColor: isRead ? 'var(--sl-surface)' : 'var(--sl-card)', border: `1px solid ${isRead ? 'var(--sl-border)' : meta.color + '30'}`, cursor: isRead ? 'default' : 'pointer', transition: 'all 0.2s' }}>
        ...
        {ann.title && <p style={{ fontSize: 13, fontWeight: 700, color: 'var(--sl-t1)', margin: '0 0 4px', lineHeight: 1.35 }}>{ann.title}</p>}
        <p style={{ fontSize: 14, fontWeight: isRead ? 400 : 600, color: 'var(--sl-t1)', margin: 0, lineHeight: 1.5 }}>{ann.message}</p>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 8 }}>
          {(ann.targetTeams ?? []).length > 0 ? <span style={{ fontSize: 10, color: 'var(--sl-t3)' }}>→ {ann.targetTeams!.join(', ')}</span> : <span style={{ fontSize: 10, color: 'var(--sl-t3)' }}>Tous les abonnés</span>}
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} style={{ borderRadius: 14, overflow: 'hidden', backgroundColor: 'var(--sl-card)', border: '1px solid var(--sl-border)' }}>
      ...
      <div style={{ padding: '12px 14px' }}>
        ...
        {ann.title && <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--sl-t1)', marginBottom: 6, lineHeight: 1.35 }}>{ann.title}</div>}
        <div style={{ fontSize: 13, color: 'var(--sl-t2)', lineHeight: 1.55, ...(isLong && !expanded ? { overflow: 'hidden', textOverflow: 'ellipsis', display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical' } : {}) }}>{ann.message}</div>
